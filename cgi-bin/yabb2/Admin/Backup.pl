...
$backupplver = 'YaBB 2.6 $Revision: 1.0 $';
...
my %dirs = (
    ...
    'html' => "yabbfiles",
    'upld' =>
      "yabbfiles/Attachments $backup_txt{'and'} yabbfiles/avatars/UserAvatars",
);
...
qq~<b>$backup_txt{33} $INFO{'backupspendtime'} $backup_txt{34}</b><br /><br />~
    ...
    $yymain .=
qq~<font color="green"><b>$backup_txt{'mailsuccess'}</b></font><br /><br />~
      if $INFO{'mailinfo'} == 1;
    $yymain .=
      qq~<font color="red"><b>$backup_txt{'mailfail'}</b></font><br /><br />~
      if $INFO{'mailinfo'} == -1;

    if ( @backup_paths == 8 ) { $allchecked = 'checked="checked" '; }

    # Yes, my checklists are really hashes. Oh well.
    foreach $item (@backup_paths) {
...
# Non-translated here, as I doubt there are words to describe "tar" in another language
    $input =
qq~name="backupmethod" id="backupmethod1" value="/usr/bin/tar" onclick="domodulecheck('/usr/bin/tar')" $methodchecklist{'/usr/bin/tar'}~;
    ...
    if ($newcommand) {
          ...
          )
        {
            ...
            $style = qq~style="font-style:italic; color:grey"~;
            $disabledtext =
              ": Tar $backup_txt{31}: $!. $backup_txt{32} " . ( $? >> 8 );
        }
    }
    else {
        $input        = qq~disabled="disabled"~;
    ...
    $input =
qq~name="backupmethod" id="backupmethod2" value="/usr/bin/zip" onclick="domodulecheck('/usr/bin/zip')" $methodchecklist{'/usr/bin/zip'}~;
    ...
    if ($newcommand) {
          ...
          )
        {
            ...
            $style = qq~style="font-style:italic; color:grey"~;
            $disabledtext =
              ": Zip $backup_txt{31}: $!. $backup_txt{32} " . ( $? >> 8 );
        }
    }
    else {
        $input        = qq~disabled="disabled"~;
...
#@backup_paths = qw(Admin_Sources Boards Languages Members Messages Templates Variables yabbfiles);
        ...
        foreach $file (
            map { $_->[0] }
            sort { $b->[1] <=> $a->[1] } map { [ $_, /(\d+)/, $_ ] } @backups
          )
        {
            ...
            $filelist .=
                qq~          <tr><td align="left">~
              . &timeformat($3)
              . qq~</td><td align="right">$filesize</td><td align="left">- ~
              . join( '<br />- ', @dirs )
              . qq~</td><td align="left">~
              . (
                $2
              ...
              . qq~$5</td><td><a href="$adminurl?action=downloadbackup;backupid=$file">$backup_txt{60}</a></td><td><a href="$adminurl?action=emailbackup;backupid=$file">$backup_txt{52}</a></td><td><a href="$adminurl?action=runbackup;runbackup_again=$1$2.0.$4.$5">$backup_txt{61}</a><br /><a href="$adminurl?action=runbackup;runbackup_again=$filename">$backup_txt{62}</a></td><td align="center">~
              . (
                ( $5 =~ /^a\.tar/ || $5 !~ /tar/ ) ? '-'
        ...
        $filelist ||=
qq~          <tr><td align="left" colspan="9"><i>$backup_txt{38}</i></td></tr>\n~;
    ...
    if ( $rememberbackup != $FORM{'rememberbackup'} ) {
        ...
        for ( $i = 0 ; $i < @settings ; $i++ ) {
            if ( $settings[$i] =~ /\$rememberbackup = \d+;/ ) {
                ...
                $settings[$i] =~
s/\$rememberbackup = \d+;/\$rememberbackup = $rememberbackup;/;
            }
        }
...
sub runbackup {
    my ( @settings, $prevmainsetting, $prevmaintext, $newmaintext,
        %pathconvert );

    if ( $INFO{'runbackup_again'} ) {
        &admin_fatal_error( "",
            "$backup_txt{32} \$INFO{'runbackup_again'}=$INFO{'runbackup_again'}"
        ) if $INFO{'runbackup_again'} !~ /^backup/;
        ...
        if ( $again[3] eq 'a' ) {
            ...
            $backupmethod =
              $again[3] eq 'tar' ? '/usr/bin/tar' : '/usr/bin/zip';
            $compressmethod =
              $again[4]
              ? ( $again[4] eq 'gz' ? '/bin/gzip' : '/bin/bzip2' )
              : 'none';
        }
        &check_backup_settings;
    }
    ...
    if ( $FORM{'backupnewest'} && $backupmethod eq '/usr/bin/zip' ) {
        my ( $day, $mon, $year );
        ( undef, undef, undef, $day, $mon, $year, undef, undef, undef ) =
...
# handling the differences in backup methods.
                    ...
                    if ( time() > $time_to_jump ) {
                        ...
                        &runbackup_loop( $i, $j, $curtime,
                            $FORM{'backupnewest'}, $backuptime );
                    }
                }
            }
            $INFO{'loop2'} = 0;
 ...
 # due to the maintenance.lock file that is removed with &automaintenance('off')
    ...
    $yySetLocation = qq~$adminurl?action=backupsettings;backupspendtime=~
      . sprintf( "%.4f", ( time() - $backuptime ) );
    &redirectexit;
}
...
sub BackupMethodInit {
    ...
    if ( $backupmethod eq 'Archive::Tar' ) {
        ...
        if ( $compressmethod eq 'Compress::Zlib' ) {    # Also using Zlib
            ...
            if ($@) {
                &admin_fatal_error( "", "$backup_txt{28} Compress::Bzip2: $@" );
            }
...
# We need this for the loops, when preventing to run into browser/server timeout.
        ...
        if ($@) {
            &admin_fatal_error( "", "$backup_txt{28} Archive::Zip: $@" );
        }
...
# We need this for the loops, when preventing to run into browser/server timeout.
        ...
        unless ( &CheckPath($backupmethod) ) {
            &admin_fatal_error( "", "$backup_txt{29} $backupmethod." );
        }
...
sub BackupDirectory {
    ...
    if ( $backupmethod eq '/usr/bin/tar' ) {
        ...
        $dir =~ s|^/||
          ; # needet not to get server log messages like "Removing leading `/' from ..."
        &ak_system(
...
# Simple subroutine to run through every entry in a directory and return a giant list of the files/subdirs.
    ...
    foreach $item (@dirlist) {
        if ( $recursemode && $item ne '.' && $item ne '..' && -d "$dir/$item" )
        {
            push( @newcontents,
...
sub downloadbackup {
    chdir($backupdir)
      || &admin_fatal_error( "", "$backup_txt{44} $backupdir", 1 );
    my $filename = $INFO{'backupid'};
    if ( $filename !~ /\Abackup/ || $filename !~ /\d{9,10}/ ) {
        &admin_fatal_error( "", $backup_txt{'45'} );
    }
    ...
    # open in binmode
    fopen( READ, $filename )
      || &admin_fatal_error( "", "$backup_txt{46} $filename", 1 );
    binmode READ;
...
sub deletebackup2 {
    ...
    unlink("$backupdir/$filename")
      || &admin_fatal_error( "", "$backup_txt{51} $backupdir/$filename", 1 );
...
~;
        ...
        $mainmessage =~
          s~LINK~$adminurl?action=downloadbackup;backupid=$filename~g;
    ...
    if ( $@ || $INFO{'linkmail'} ) {
        ...
        $mainmessage =~
          s~LINK~$adminurl?action=downloadbackup;backupid=$filename~;
   ...
   <table width="100%" cellspacing="1" cellpadding="10">
    ...
    foreach ( split( /\n/, qx($recovertype) ) ) {
        ...
        if ( !$checkdir{$1} && $2 ) {
            $checkdir{$1} = 1;
            $yymain .=
qq~           <tr><td align="left">/$1 *$backup_txt{114}</td><td align="center"><input type="text" name="u-$1" value="6" size="1" maxlength="1" onkeyup="CheckCHMOD(this.value,6,this);" /> <input type="text" name="g-$1" value="6" size="1" maxlength="1" onkeyup="CheckCHMOD(this.value,6,this);" /> <input type="text" name="a-$1" value="" size="1" maxlength="1" onkeyup="CheckCHMOD(this.value,0,this);" /></td></tr>\n~;
        }
    }
...
sub recoverbackup2 {
    ...
    if ( $FORM{'originalrestore'} ) {
        ...
        chmod(
            oct("0$FORM{'u-newdir'}$FORM{'g-newdir'}$FORM{'a-newdir'}"),
            $restore_root
        );    # mkdir somtimes does not set the CHMOD as expected
    }
