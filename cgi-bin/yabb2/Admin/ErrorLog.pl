...
$errorlogplver = 'YaBB 2.6 $Revision: 1.0 $';
...
qq~<a href="$scripturl?action=viewprofile;username=$useraccount{$tmp_user}" target ="_blank">$userprofile{$tmp_user}->[1]</a>~;
        ...
        {
            $tmp_error = $tmp_error
              . " - (<span style=\"color: #FF0000;\">$tmp_password</span>)";
        }

        $b++;
        $addel =
qq~<td class="windowbg" align="center"><input type="checkbox" name="error$tmp_id" value="$tmp_id" class="windowbg" style="border: 0px;" /></td>~;
        $actualnum++;
        $print_errorlog .= qq~
      <tr>
...
$print_errorlog
    ...
    if ( $errorcount > 0 ) {
        $yymain .=
qq~<input type="checkbox" name="checkall" id="checkall" class="windowbg" style="border: 0px;" onclick="if (this.checked) checkAll(); else uncheckAll();" />~;
    }
    $yymain .= qq~
         </td>
     </tr>
...
sub CleanErrorLog {
    ...
    if ( -e ("$vardir/errorlog.txt") ) {
        unlink("$vardir/errorlog.txt") || die "$!";
    }
...
sub DeleteError {
    &is_admin_or_gmod;
    my ( $count, $memnum, $currentmem, @deademails, $start, $sortmode,
        $sortorder );
    chomp $FORM{"button"};
