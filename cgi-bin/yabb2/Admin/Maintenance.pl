...
$maintenanceplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub RebuildMemHistory {
    ...
    while (@contents) {
        ...
        my %posts = ();
        foreach (@topic) {
            ( undef, undef, undef, $mdate, $user, undef ) =
              split( /\|/, $_, 6 );
            if ( $user ne 'Guest' ) {
                $posts{$user}++;
                $dates{$user} = $mdate;
...
sub RebuildNotifications {
    ...
    if ( !%members ) {
        opendir( MEMBNOTIF, $memberdir )
          || &fatal_error( 'cannot_open', "$memberdir", 1 );
        map { $_ =~ /(.+)\.(vars|wait|pre)$/; $members{$1} = $2; }
          grep { /.\.(vars|wait|pre)$/ } readdir(MEMBNOTIF);
        closedir(MEMBNOTIF);
    ...
    if ( !$exitloop ) {
        ...
        while (@tmaildir) {
            ...
            foreach my $user (@temp) {
                ...
                my %t;
                foreach (
                    split( /,/, ${ $uid . $user }{'thread_notifications'} ) )
                {
                    $t{$_} = 1;
                }
                $t{$mythread} = 1;
