...
$managecatsplver = 'YaBB 2.6 $Revision: 1.0 $';
...
<table border="0" align="center" cellspacing="1" cellpadding="4" width="100%">~;
    ...
    for ( $i = 0 ; $i < $FORM{'amount'} ; $i++ ) {
        if (   ( !$editcats[$i] && $INFO{"action"} eq "catscreen" )
            || ( $editcats[$i] eq "" && $INFO{"action"} eq "catscreen" ) )
        {
            next;
        }
        if ( $INFO{"action"} eq "catscreen" ) {
            ...
            foreach $catid (@categoryorder) {
                ...
                &ToChars($curcatname);
                $cattext = $curcatname;
                if ( $catallowcol eq '' || $catallowcol eq '1' ) {
  ...
  <tr>
    ...
    <td class="windowbg2"><br /><input type="text" name="catimage$i" id="catimage$i" value="$catimage" size="40" />~
          . (
            $catimage
            ? qq~<br /><br  /><img src="$catimage" alt="" border="0" />~
            : ''
          )
          . qq~<br /><br /></td>
  </tr>~;
    ...
    for ( $i = 0 ; $i < $FORM{'amount'} ; $i++ ) {
        if ( $FORM{"catimage$i"} ne "" ) {
            &admin_fatal_error( "invalid_character", $FORM{"catimage$i"} )
              if $FORM{"catimage$i"} =~ /[^0-9a-zA-Z_\.#\%\-:\+\?\$&~,\@\/]/;
            &admin_fatal_error( "", $admintxt{'44'} )
              if $FORM{"catimage$i"} !~ /\.(gif|png|jpe?g)$/;
        }
