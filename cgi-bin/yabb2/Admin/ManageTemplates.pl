...
$managetemplatesplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub ModifyCSS {
    ...
    ( $aktstyle, $aktimages, $akthead, $aktboard, $aktmessage, $aktdisplay ) =
      split( /\|/, $templateset{"$akttemplate"} );
    ...
    $stylestr =~ s/\s{2,}/ /g;
    my (
        $selstyl,        $selhidden,          $postsstyle,
        $seperatorstyle, $bodycontainerstyle, $bodystyle,
        $containerstyle, $titlestyle,         $titlestyle_a,
        $categorystyle,  $categorystyle_a,    $window1style,
        $window2style,   $inputstyle,         $textareastyle,
        $selectstyle,    $quotestyle,         $codestyle,
        $editbgstyle,    $highlightstyle,     $gen_fontsize
    );

    $gen_fontsize =
qq~<select name="cssfntsize" id="cssfntsize" style="vertical-align: middle;" onchange="previewFont()">~;
    for ( $i = 7 ; $i < 21 ; $i++ ) {
        $gen_fontsize .= qq~<option value="$i">$i</option>~;
    }
    $gen_fontsize .= qq~</select>~;
    $gen_fontface =
qq~<select name="cssfntface" id="cssfntface" style="vertical-align: middle;" onchange="previewFontface()">
    ...
    $gen_borderweigth =
qq~<select name="borderweigth" id="borderweigth" style="vertical-align: middle;" onchange="previewBorder()">~;
    for ( $i = 0 ; $i < 6 ; $i++ ) {
...
<br /><br />
    ...
    $yymain .= qq~
      ...
      . ( split( /\./, $cssfile ) )[0] . qq~" size="30" maxlength="30" />
...
sub ModifyCSS2 {
    ...
    if ( $FORM{'button'} == 1 ) {
        ...
        if (   $style_name !~ m^\A[0-9a-zA-Z_\.\#\%\-\:\+\?\$\&\~\.\,\@/]+\Z^
            || $style_name eq "" )
        {
...
sub ModifySkin {
    ...
    if ( $selectedsection eq "vboard" ) { $boardsel = qq~ checked="checked"~; }
        ...
        $messagesel = qq~ checked="checked"~;
    }
    elsif ( $selectedsection eq "vdisplay" ) {
        $displaysel = qq~ checked="checked"~;
    }
    ...
    foreach $name ( sort @templates ) {
        ...
        foreach $file (@templatefiles) {
            ...
            if ( $ext eq 'html' ) {
                if ( $file eq $headfile ) {
                    $hselected = qq~ selected="selected"~;
                    $viewhead  = $name;
                }
                $headtemplates .=
            ...
            if ( $section eq 'BoardIndex' ) {
                if ( $thefile eq $boardfile ) {
                    $bselected = qq~ selected="selected"~;
                    $viewboard = $name;
                }
                $boardtemplates .=
                ...
                if ( $thefile eq $messagefile ) {
                    $mselected   = qq~ selected="selected"~;
                    $viewmessage = $name;
                }
                $messagetemplates .=
                ...
                if ( $thefile eq $displayfile ) {
                    $dselected   = qq~ selected="selected"~;
                    $viewdisplay = $name;
                }
                $displaytemplates .=
                ...
                if ( $thefile eq $mycenterfile ) {
                    $myselected   = qq~ selected="selected"~;
                    $viewmycenter = $name;
                }
                $mycentertemplates .=
...
qq~$tabsep<span title="$img_txt{'108'}">$tabfill$img_txt{'108'}$tabfill</span>$tabsep~;
    $tempmenu =~
s~img src\=\"$imagesdir\/(.+?)\"~&TmpImgLoc($1, $tempimages, $tempimagesdir)~eisg;
    $temp21menu =
...
s~img src\=\"$imagesdir\/(.+?)\"~&TmpImgLoc($1, $tempimages, $tempimagesdir)~eisg;
    $rssbutton =
qq~<img src="$imagesdir/rss.png" border="0" alt="" style="vertical-align: middle;" />~;
    ...
    my $tempsearchbox =
qq~<input type="text" name="search" size="16" style="font-size: 11px; vertical-align: middle;" />~;
    $tempsearchbox .=
qq~<img src="$imagesdir/search.gif" alt="" style="border: 0; background-color: transparent; margin-right: 5px; vertical-align: middle;" />~;
    ...
    $fulltemplate =~
s~img src\=\"$tempimages\/(.+?)\"~&TmpImgLoc($1, $tempimages, $tempimagesdir)~eisg;
    $fulltemplate =~
      s/<a href="http:\/\/validator.w3.org\/check\/referer">.+?<\/a>//g;
    $fulltemplate =~
s/<a href="http:\/\/jigsaw.w3.org\/css\-validator\/validator\?uri\=<yabb url>">.+?<\/a>//g;
    $fulltemplate =~ s~[\n\r]~~g;
    &ToHTML($fulltemplate);
...
sub ModifySkin2 {
    ...
    if ( $FORM{'button'} == 1 ) {
        $yySetLocation =
qq~$adminurl?action=modskin;templateset=$formattemp;cssfile=$FORM{'cssfile'};imgfolder=$FORM{'imgfolder'};headfile=$FORM{'headfile'};boardfile=$FORM{'boardfile'};messagefile=$FORM{'messagefile'};displayfile=$FORM{'displayfile'};mycenterfile=$FORM{'mycenterfile'};menutype=$FORM{'menutype'};selsection=$FORM{'selsection'}~;
        ...
        if ( $template_name !~ m^\A[0-9a-zA-Z_\ \.\#\%\-\:\+\?\$\&\~\.\,\@/]+\Z^
            || $template_name eq "" )
        {
            &admin_fatal_error("invalid_template");
        }
...
qq~<div class="small" style="float: left; width: 49%;"><span style="color: $color;"><b>lllll</b></span> $title</div>~;
    ...
    ( $title, undef, undef, $color, $noshow ) =
      split( /\|/, $Group{'Global Moderator'}, 5 );
    if ( $color && $noshow != 1 ) {
        $grpcolors .=
qq~<div class="small" style="float: left; width: 49%;"><span style="color: $color;"><b>lllll</b></span> $title</div>~;
    }
...
qq~$boardindex_txt{'63'}: $templ_txt{'74'}<br />$boardindex_txt{'63a'}: $templ_txt{'74a'}~;
    ...
    $tempcatlink =
qq~<img src="$_[1]/cat_collapse.gif" alt="" border="0" /> <a href="javascript:;">$templ_txt{'81'}</a>~;
...
qq~$boardindex_txt{'791'} <select style="font-size: 7pt;"><option>&nbsp;</option><option>5</option></select> $boardindex_txt{'792'} $boardindex_txt{'793'}~;
    ...
    my $tempbotlist =
      qq~<span class="small">Googlebot (1), MSN Search (2)</span>~;
    my $tempuserson =
...
qq~<span class="small" style="color: $admcolor;"><b>${$uid.$username}{'realname'}</b></span><br />~;
    ...
    my $tempboardpic =
      qq~ <img src="$imagesdir/boards.gif" alt="$tempcurboard" border="0" />~;
...
qq~$boardindex_txt{'236'} <a href="javascript:;"><b>$templ_txt{'80'}</b></a>~;
    ...
    $boardindex_template =~
      s~img src\=\"$tmpimagesdir\/(.+?)\"~&TmpImgLoc($1, $_[1], $_[2])~eisg;
    ...
    $boardindex_template =~
      s~img src\=\"$_[1]\/(.+?)\"~&TmpImgLoc($1, $_[1], $_[2])~eisg;
...
qq~($messageindex_txt{'298'}: $templ_txt{'74'} - $messageindex_txt{'298a'}: $templ_txt{'74a'})~;
    ...
    my $temppageindextgl =
qq~<img src="$_[1]/xx.gif" style="vertical-align: middle;" alt="" border="0" />~;
    my $temppageindex =
qq~<span class="small" style="vertical-align: middle;"> <b>$messageindex_txt{'139'}:</b> 1</span>~;
    my $tempthreadpic =
qq~<img src="$_[1]/thread.gif" style="vertical-align: middle;" alt="" border="0" />~;
    my $tempmicon =
qq~<img src="$_[1]/xx.gif" style="vertical-align: middle;" alt="" border="0" />~;
    my $tempnew =
qq~<img src="$_[1]/new.gif" style="vertical-align: middle;" alt="" border="0" />~;
    my $tempmsublink = $templ_txt{'83'};
    my $tempmname    = ${ $uid . $username }{'realname'};
    my $templastpostlink =
qq~<img src="$_[1]/lastpost.gif" style="vertical-align: middle;" alt="" border="0" /> $templ_txt{'82'}~;
    ...
    $bdpic =
qq~ <img src="$_[1]/boards.gif" alt="$templ_txt{'72'}" border="0" align="middle" /> ~;
    $message_permalink = $messageindex_txt{'10'};
    $temp_attachment =
      qq~<img src="$_[1]/paperclip.gif" alt="$messageindex_txt{'5'}" />~;
    ...
    $messageindex_template =~
      s/({|<)yabb pageindex bottom(}|>)/$temppageindex1/g;
    ...
    $messageindex_template =~
      s/({|<)yabb pageindex toggle(}|>)/$temppageindextgl/g;
    ...
    $messageindex_template =~
      s/({|<)yabb admin icons(}|>)/$tempyabbadminicons/g;
...
sub DisplayTempl {
    ...
    if ( $UseMenuType == 0 ) {
        ...
        $yimimg =
qq~$menusep<img src="$yyhtml_root/Buttons/$language/yim.png" alt="" border="0" />~;
        $aimimg =
qq~$menusep<img src="$yyhtml_root/Buttons/$language/aim.png" alt="" border="0" />~;
        $msnimg =
qq~$menusep<img src="$yyhtml_root/Buttons/$language/msn.png" alt="" border="0" />~;
    }
...
qq~<span class="small" style="vertical-align: middle;"> <b>$display_txt{'139'}:</b> 1</span>~;
    ...
    my $template_threadimage =
      qq~<img src="$_[1]/thread.gif" align="middle" alt="" />~;
...
qq~<span style="color: $color;"><b>${$uid.$username}{'realname'}</b></span><br />~;
    ...
    my $avatar =
qq~<img src="$facesurl/elmerfudd.gif" name="avatar" alt="" border="0" align="middle" style="max-width: $userpic_width\px\; max-height: $userpic_height\px" />~;
    my $message =
    ...
    my $ipimg =
      qq~<img src="$imagesdir/ip.gif" alt="" border="0" align="middle" />~;
...
sub UpdateTemplates {
    ...
    if ( $tempjob eq "new" ) {    # update to new style from very old versions
        ...
        if ($template_mycenter) {
            $templateset{"$testname"} .= "|$tempelement";
        }
        else { $templateset{"$testname"} .= "|default"; }
        if ($template_menutype) {
            $templateset{"$testname"} .= "|$tempelement";
        }
