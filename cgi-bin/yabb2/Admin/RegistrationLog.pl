...
$registrationlogplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub view_reglog {
    ...
    if ( @logentries > 0 ) {
        ...
        if ( $startpage > 0 ) {
            $pageindex =
qq~<a href="$adminurl?action=$action;newstart=0" style="font-weight: normal;">1</a>&nbsp;...&nbsp;~;
        }
        if ( $startpage == 25 ) {
            $pageindex =
qq~<a href="$adminurl?action=$action;newstart=0" style="font-weight: normal;">1</a>&nbsp;~;
        ...
        for ( $counter = $startpage ; $counter < $endpage ; $counter += 25 ) {
            $pageindex .=
              $newstart == $counter
              ? qq~<b>$tmpa</b>&nbsp;~
              : qq~<a href="$adminurl?action=$action;newstart=$counter" style="font-weight: normal;">$tmpa</a>&nbsp;~;
            $tmpa++;
        }
...
sub approve_registration {
    ...
    if ( -e "$memberdir/$apruser.wait" && $foundmember ne "" ) {
        ...
        {
            $yymain .=
qq~<font color="red"><b>$prereg_txt{'email_taken'} <i>${$uid.$apruser}{'email'}</i> ($prereg_txt{'35'}: $apruser)</b></font>~;
            &view_reglog;
        }
