...
$smiliesplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub SmiliePanel {
    ...
    foreach $line ( sort { uc($a) cmp uc($b) } @contents ) {
            ...
            || $extension =~ /png/i )
        {
            if ( $line !~ /banner/i ) {
                $smilieslist .= qq~<tr>
    <td class="windowbg2" width="5%" align="center"><input type="radio" name="showinbox" value="$name"~
                  . ( $showinbox eq $name ? ' checked="checked"' : '' )
                  . qq~ /></td>
...
<table border="0" width="98%" cellspacing="1" cellpadding="4" class="bordercolor" align="center">
  <tr>
    ...
    foreach (@SmilieURL) {
        if ( $i != 0 ) {
            $up =
qq~<a href="$adminurl?action=smiliemove;index=$i;moveup=1"><img src="$imagesdir/smiley_up.gif" border="0" alt="$asmtxt{'13'}" title="$asmtxt{'13'}" /></a>~;
        }
        else {
        ...
        if ( $SmilieURL[ $i + 1 ] ) {
            $down =
qq~<a href="$adminurl?action=smiliemove;index=$i;movedown=1"><img src="$imagesdir/smiley_down.gif" border="0" alt="$asmtxt{'14'}" title="$asmtxt{'14'}" /></a>~;
        }
        else {
            $down =
              qq~<img src="$imagesdir/smiley_down.gif" border="0" alt="" />~;
        }
        $yymain .= qq~<tr>
    <td class="windowbg2" width="5%" align="center"><input type="radio" name="showinbox" value="$SmilieDescription[$i]"~
          . ( $showinbox eq $SmilieDescription[$i] ? ' checked="checked"' : '' )
          . qq~ /></td>
    ...
    <td class="windowbg2" width="15%" align="center"><input type="checkbox" name="smbox[$i]" value="1"~
          . ( $SmilieLinebreak[$i] eq "<br />" ? " checked='checked'" : "" )
          . qq~ /></td>
    <td class="windowbg2" width="10%" align="center"><img src="~
          . (
              $SmilieURL[$i] =~ /\//i
            ? $SmilieURL[$i]
    ...
    <td class="windowbg2" colspan="4" align="right"><input type="radio" name="showinbox" id="showinbox" value=""~
              . ( !$showinbox ? ' checked="checked"' : '' ) . qq~ /></td>
  </tr><tr>
