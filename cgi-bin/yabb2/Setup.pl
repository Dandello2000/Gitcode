...
$setupplver = 'YaBB 2.6 $Revision: 1.0 $';
    ...
    if ( $action && !$INFO{'convert'} ) {
      ...
      <form action="$set_cgi?action=prepare" method="post">
            ...
            <tr valign="middle">
                  ...
                  Make sure your YaBB 2.6 installation is running and that it has all the correct folder paths and URLs.<br />
                  Proceed through the following steps to convert your YaBB 1 Gold - SP 1.x forum to YaBB 2.6!<br /><br />
                  <b>If</b> your YaBB 1 Gold - SP 1.x forum is located on the same server as your YaBB 2.6 installation:
                  <ol>
                  <li>Insert the path to your YaBB 1 Gold - SP 1.x forum in the input field below</li>
                  <li>Click on the 'Continue' button</li>
                  </ol>
                  <b>Else</b> if your YaBB 1 Gold - SP 1.x forum is located on a different server than your YaBB 2.6 installation or if you do not know the path to your SP 1.x forum:
                  <ol>
                  ...
                  Convert/Members, and Convert/Messages folders of your YaBB 2.6 installation, and chmod them 777.</li>
                  <li>Copy cat.txt from the /Variables folder of your YaBB 1 Gold - SP 1.x installation to the Convert/Variables folder of your YaBB 2.6 installation, and chmod it 666.</li>
                  <li>If you have 'Add More Membergroups' installed on your YaBB 1 Gold - SP 1.x, copy MemberStats.txt from the /Variables folder of your YaBB 1 Gold - SP 1.x installation to the Convert/Variables folder of your YaBB 2.6 installation, and chmod it 666.</li>
...
"Member conversion (members2) 'mstart1' ($INFO{'mstart1'}), 'mstart2' ($INFO{'mstart2'}) error!"
      ...
      <div class="bordercolor" style="padding: 0px; width: 100%; margin-left: 0px; margin-right: 0px;">
          ...
          . ( $time_to_jump - $INFO{'starttime'} ) . qq~ seconds</i>.<br />
                  You are converting <i>~
          . int( ( $INFO{'st'} + 60 ) / 60 ) . qq~ minutes</i>.<br />
                  <br />
                  There are <b>~
          ...
          . qq~/$INFO{'mtotal'}</b> Members left to be converted.<br />
...
"Date & Time conversion (dates2) error! pollfile($INFO{'pollfile'}), polledfile($INFO{'polledfile'})",
      ...
      <div class="bordercolor" style="padding: 0px; width: 100%; margin-left: 0px; margin-right: 0px;">
          ...
          . int( ( $INFO{'st'} + 60 ) / 60 ) . qq~ minutes</i>.<br />
                  ...
                  <div style="float: left; width: 350px; height: 14px;">There are <b>~
          . ( $INFO{'totalpolls'} - $INFO{'pollfile'} )
      ...
      // -->
        ...
        if ( !$INFO{'clean'} ) {
            fopen( FORUMTOTALS, ">>$boardsdir/forum.totals" )
              || &setup_fatal_error( "Can not open $boardsdir/forum.totals",
                1 );
            foreach my $testboard (@allboards) {
                ...
                if ( -e "$boardsdir/$testboard.ttl" ) {
                    fopen( BOARDTTL, "$boardsdir/$testboard.ttl" )
                      || &setup_fatal_error(
                        "Can not open $boardsdir/$testboard.ttl", 1 );
                    my $line = <BOARDTTL>;
                    fclose(BOARDTTL);
                    chomp $line;
...
qq~<br /><br />There were some illegal usernames. Their names were changed. Please inform them. You can find the list in the $vardir/fixusers.txt~;
      ...
      <div class="bordercolor" style="padding: 0px; width: 100%; margin-left: 0px; margin-right: 0px;">
          ...
          . int( ( $INFO{'st'} + 60 ) / 60 ) . qq~ minutes</i>.<br />
                  ...
                  <br />
                  <br />
                  You may now login to your forum. Enjoy using YaBB 2.6!
                  </td>
            </tr>
            <tr>
...
"Clean Up (cleanup2) error! pass_error($INFO{'pass_error'}), my_re_tot($INFO{'my_re_tot'}), memb_index($INFO{'memb_index'}), my_mail_n($INFO{'my_mail_n'})",
    ...
    $yyim    = 'You are running the YaBB 2.6 Converter.';
    $yytitle = 'YaBB 2.6 Converter';
    &SetupTemplate;
}
...
"The CHMOD of the $datadir is not set correctly! Cannot read this directory! ",
    ...
    foreach $file (@boardlist) {
        unless ( $file eq ".htaccess"
            || $file eq "index.html"
            || $file eq "forum.control"
            || $file eq "."
            || $file eq ".." )
        {
...
sub ConvertMembers1 {
    fopen( MEMDIR, "$convmemberdir/memberlist.txt" )
      || &setup_fatal_error( "$maintext_23 $convmemberdir/memberlist.txt: ",
        1 );
    my @memlist = <MEMDIR>;
    fclose(MEMDIR);
...
sub IllegalUser {
    ...
    if ( -e "$convmemberdir/$user.om" ) {
        fopen( MSNFILE, "$convmemberdir/$user.om" )
          || &setup_fatal_error( "$maintext_23 $convmemberdir/$user.om: ", 1 );
    ...
    if ( -e "$convmemberdir/$user.ll" ) {
        ...
        $lastonline =~
s~(\d{1,2}\/\d{1,2}\/\d{2,4}).*?(\d{1,2}\:\d{1,2}\:\d{1,2})~&conv_stringtotime("$1 at $2")~eis;
        $lastpost =~
s~(\d{1,2}\/\d{1,2}\/\d{2,4}).*?(\d{1,2}\:\d{1,2}\:\d{1,2})~&conv_stringtotime("$1 at $2")~eis;
        $lastim =~
s~(\d{1,2}\/\d{1,2}\/\d{2,4}).*?(\d{1,2}\:\d{1,2}\:\d{1,2})~&conv_stringtotime("$1 at $2")~eis;
    ...
    $regitime =~
s~(\d{2}\/\d{2}\/\d{2,4}).*?(\d{2}\:\d{2}\:\d{2})~&conv_stringtotime("$1 at $2")~eis;
    ...
    if ( $settings[5] ) {
        ...
        $settings[5] =~ s/\"/&quot;/g;
        $settings[5] =~
s~\[size=([+-]?\d)\](.*?)\[/size\]~ "\[size=" . &conv_size($1) . "\]$2\[/size\]" ~ige;
    ...
    %{ $uid . $fixeduser } = (
        ...
        'webtitle' => "$settings[3]",
        'weburl'   => (
            ( $settings[4] && $settings[4] !~ m~\Ahttps?://~ ) ? "http://" : ""
          )
          . $settings[4],
                    ...
                    for ( my $i = 0 ; $i < @divfiles ; $i++ ) {
                        if ( $cnt == 2 ) {    # imstore
                            ...
                            $name =
                              ...
                              : $name;
                            $date =~
s~(\d{1,2}\/\d{1,2}\/\d{2,4}).*?(\d{1,2}\:\d{1,2}\:\d{1,2})~&conv_stringtotime("$1 at $2")~ei;
                            $message =~ s~<br>~<br />~ig;
                            if ( $folder eq 'outbox' ) {
                                $folder = 'out';
...
"$cntcat|$foundboard|$cntpic|$cntdescription|$cntmods|$cntmodgroups|$cntstartperms|$cntreplyperms|$cntpollperms|$cntzero|$cntpassword|$cnttotals|$cntattperms|$spare|||\n"
        ...
        {    # add trash if not exists
            push( @boardcontrol,
"staff|recycle||If the Recycle Bin is turned on, removed topics will be moved to this board. This will allow you to recover them if it is necessary.  You should purge messages in this board frequently to keep it clean.|admin|||||1|||1||||\n"
            );
...
sub ConvertBoards {
    ...
    for ( my $i = ( $INFO{'bstart'} || 0 ) ; $i < @boards ; $i++ ) {
        fopen( BOARDFILE, "$convboardsdir/$boards[$i].txt" )
          || &setup_fatal_error(
            "$maintext_23 $convboardsdir/$boards[$i].txt: ", 1 );
        @boardfile = <BOARDFILE>;
        fclose(BOARDFILE);
...
sub ConvertMessages {
      ...
      )
    {
        ...
        for (
            my $tops = ( $INFO{'tcount'} || 0 ) ;
            $tops < $totalmess ;
            $tops++
          )
        {
            ( $thread, undef, undef, undef, undef, $replies, undef ) =
              split( /\|/, $brdmessageline[$tops], 7 );
            ...
            @temparray = ();
            my (
                $subject,  $name,   $email,   $mdate,   $musername,
                $icon,     $dummy,  $user_ip, $message, $ns,
                $editdate, $editby, $attachment
            );
            foreach $msgline (@messagelines) {
                ...
                {    # too many RegExpr take too much time!!!
                    $message =~
s~\[quote(\s+author=(.*?)\s+link=(.*?)\s+date=(.*?)\s*)?\](.*?)\[/quote\]~&QuoteFix($2,$3,$4,$5)~eig;
                    $message =~
s~\[(glow|shadow)=.*?\](.*?)\[/(glow|shadow)\]~\[glb\]$2\[/glb\]~ig;
                    $message =~
s~\[size=([+-]?\d)\](.*?)\[/size\]~ "\[size=" . &conv_size($1) . "\]$2\[/size\]" ~ige;
                }
                $message =~ s~<br>~<br />~ig;
    ...
    for ( my $t = $start ; $t < $total ; $t++ ) {
        ...
        foreach my $user ( keys %{ $uid . $thread } ) {
            fopen( RLOG, ">>$memberdir/$user.rlog" )
              || &setup_fatal_error( "$maintext_23 $memberdir/$user.rlog: ",
                1 );
            print RLOG
...
sub MyMemberIndex {
    ...
    for ( my $j = ( $INFO{'memb_index'} || 0 ) ; $j < $totalmemb ; $j++ ) {
        ...
        $memberlist{$member} =
          sprintf( "%010d", ${ $uid . $member }{'regtime'} );
        $memberinf{$member} =
...
sub format_timestring {
    ...
    if ( $time_string !~
        m~(\d{1,2})\/(\d{1,2})\/(\d{2,4}).*?(\d{1,2})\:(\d{1,2})\:(\d{1,2})~is )
    {
...
sub autoconfig {
    ...
    if ( !$iamadmin ) {
        &setup_fatal_error(
qq~Setup Error: You have no access rights to this function. Only user "admin" has if logged in!~
        );
    }
...
EOF
        ...
        fopen( SETTING, ">$varsdir/gmodsettings.txt" )
          || &setup_fatal_error( "$maintext_23 $varsdir/gmodsettings.txt: ",
            1 );
        print SETTING &nicely_aligned_file($setfile);
        fclose(SETTING);
    }
    ...
    if ( !-e "$varsdir/news.txt" ) {
        ...
        print NEWSFILE "We've upgraded to YaBB 2.6!\n";
        print NEWSFILE
...
"Latest info can be found on the [url=http://www.yabbforum.com/community/]YaBB Chat and Support Community[/url].\n";
    ...
    if ( !-e "$varsdir/registration.log" ) {
        fopen( REGLOG, ">$varsdir/registration.log" )
          || &setup_fatal_error( "$maintext_23 $varsdir/registration.log: ",
            1 );
        print REGLOG "";
        fclose(REGLOG);
    }
...
~;
    ...
    $yyim    = "You are running YaBB 2.6 Setup.";
    $yytitle = "YaBB 2.6 Setup";
    &SetupTemplate;
}
 ...
 <div class="bordercolor" style="padding: 0px; width: 100%; margin-left: 0px; margin-right: 0px;">
   <table width="100%" cellspacing="1" cellpadding="4">
      <tr valign="middle">
            ...
            Here you can set some of the default settings for your new YaBB 2.6 forum.<br />
      ...
      <br />
            ...
            <div style="float: left; font-family: verdana; width: 55%; text-align: left; font-size: 12px; padding-top: 2px; padding-bottom: 2px;">
                  <span style="font-size:small">Your actually displayed local time:<br /><b>~
      . &timeformat( $date, 4 )
      . qq~<br /><br /></span><select name="usertimesign"><option value="">+</option><option value="-">-</option></select>
    ...
    for ( my $i = 0 ; 60 > $i ; $i++ ) {
        ...
        $j = ( split( /\./, $j ) )[1] || 0;
        $yymain .=
          qq~<option value="$j">~ . sprintf( "%02d", $i ) . qq~</option>~;
    }
    $yymain .= qq~</select>
            </div>
...
'We are currently upgrading our forum again. Please check back shortly!';
        ...
        $max_log_days_old       = 90;
        $fadertime              = 1000;
        $defaultusertxt         = 'I Love YaBB 2.6!';
        $timeout                = 5;
        $HotTopic               = 10;
        $VeryHotTopic           = 25;
        $barmaxdepend           = 0;
...
sub CheckInstall {
    ...
    if ( !-e "$vardir/attachments.txt" ) {
        $var_missing .= qq~attachments.txt, ~;
    }
    ...
    if ( !-e "$vardir/gmodsettings.txt" ) {
        $var_missing .= qq~gmodsettings.txt, ~;
    }
    ...
    if ( !-e "$vardir/registration.log" ) {
        $var_missing .= qq~registration.log, ~;
    }
...
~;
    ...
    if ($set_created) {
      ...
      Click on 'Continue' and go to your <i>Admin Center - Forum Settings</i> to set the options for your YaBB 2.6 forum.<br />
      Click on 'Convert' to convert your YaBB 1 Gold - SP 1.x forum to YaBB 2.6.
      </td></tr>
            ~;
    }
...
sub ready {
    if ( $INFO{'nextstep'} eq 'Setup' ) {
        $yySetLocation = qq~$INFO{'nextstep'}.$yyext?convert=1~;
    }
    elsif ( -e "$INFO{'nextstep'}.$yyext" ) {
        &UpdateCookie("delete");
...
sub CreateConvLock {
    ...
    print LOCKFILE
      q~It prevents it being run again after it has been run once.\n~;
...
qq~<img src="$imagesdir/tabfill.gif" border="0" alt="" style="vertical-align: middle;" />~;
        ...
        $NavLink6 =
          qq~$tabsep<span>$tabfill Login $tabfill</span>$tabsep&nbsp;~;
...
<div class="bordercolor" style="padding: 0px; width: 100%; margin-left: 0px; margin-right: 0px;">
    ...
    $yyim    = "YaBB 2.6 Setup and Converter has already been run.";
    $yytitle = "YaBB 2.6 Setup/Converter";
    &SetupTemplate;
}
...
<center><br /><a href="javascript:history.go(-1)">Back</a></center>
      ~;
    $yyim    = "YaBB 2.6 Convertor Error.";
    $yytitle = "YaBB 2.6 Convertor Error.";
...
qq~<link rel="stylesheet" href="$forumstylesurl/$usestyle.css" type="text/css" />~;
    ...
    $yyuname =
      $iamguest ? '' : qq~$maintxt{'247'} ${$uid.$username}{'realname'}, ~;
    ...
    for ( my $i = 0 ; $i < @yytemplate ; $i++ ) {
        ...
        if (
            !$yycopyin
            && (   $curline =~ m~<yabb copyright>~
                || $curline =~ /{yabb copyright}/ )
          )
        {
            $yycopyin = 1;
        }
...
sub nicely_aligned_file {
    my $filler = ' ' x
      50;    # Make files look nicely aligned. The comment starts after 50 Col
