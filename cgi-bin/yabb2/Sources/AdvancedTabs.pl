...
$advancedtabsplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub AddNewTab2 {
    if ($iamadmin) {
        ...
        if (   $taburl =~ /$boardurl\/$yyexec\.$yyaext/i
            && $taburl =~ /action\=(.*?)(\;|\Z)/i )
        {
            $taburl      = 1;
            $tabaction   = $1;
        ...
        elsif ($taburl =~ /$boardurl\/AdminIndex\.$yyaext/i
            && $taburl =~ /action\=(.*?)(\;|\Z)/i )
        {
            $taburl      = 2;
            $tabaction   = $1;
        ...
        $tabaction =~ s/\W/_/g;
        map { &fatal_error( 'tabext', $tabaction ) if $_ =~ /^$tabaction\|?/; }
          @AdvancedTabs;
...
sub EditTab {
    $tabsave =
qq~<img src="$imagesdir/tabsave.gif" border="0" alt="$tabmenu_txt{'savetab'}" title="$tabmenu_txt{'savetab'}" style="vertical-align: middle;" />~;
    $tabdel =
qq~<img src="$imagesdir/tabdelete.gif" border="0" alt="$tabmenu_txt{'tabdel'}" title="$tabmenu_txt{'tabdel'}" style="vertical-align: middle;" />~;
    ...
    for ( my $i = 0 ; $i < @AdvancedTabs ; $i++ ) {
        if ( $AdvancedTabs[$i] =~ /\|/ ) {
                ...
                || $iamadmin )
            {
                ...
                $edittab{$tab_key} =
qq~<form action="$scripturl?action=edittab2;savetab=$enc_key" method="post" name="$tab_key$isexttabs" style="display: inline; white-space: nowrap;">~;
...
qq~ <a href="$scripturl?action=deletetab;deltab=$enc_key">$tabdel</a>~;
                ...
                $edittabs .=
                  ...
                  . (
                    $tab_key eq $INFO{"thetab"} ? ' selected="selected"' : '' )
...
sub DeleteTab {
    if ($iamadmin) {
        ...
        foreach $lngdir (@languages) {
            ...
            fopen( TABTXT, "$langdir/$lngdir/tabtext.txt" )
              || &fatal_error( 'file_not_open',
                "$langdir/$lngdir/tabtext.txt" );
            %tabtxt = map /(.*)\t(.*)/, <TABTXT>;
            fclose(TABTXT);
