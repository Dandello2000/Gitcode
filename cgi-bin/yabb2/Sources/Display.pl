...
$displayplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub Display {
    ...
    if ( &AccessCheck( $currentboard, '', $boardperms ) ne "granted" ) {
    ...
    my (
        ...
        $notify,           $max,               $start,
        $windowbg,         $mattach,           $pagedropindex,
        $template_viewers, $template_favorite, $template_pollmain,
        $navback,          $mark_unread,       $pollbutton,
        $icanbypass,       $replybutton,       $bypassReplyButton
    );
...
qq~$menusep<a href="$scripturl?action=post;num=$viewnum;virboard=$vircurrentboard;title=AddPoll">$img{'addpoll'}</a>~;
    ...
    if ( substr( $INFO{'start'}, 0, 3 ) eq 'all' && $showpageall != 0 ) {
        ...
        $all               = 1;
        $allselected       = qq~ selected="selected"~;
    ...
    $pageindex1 =
qq~<span class="small" style="float: left; height: 21px; margin: 0px; margin-top: 2px;"><img src="$imagesdir/index_togl.gif" border="0" alt="$display_txt{'19'}" title="$display_txt{'19'}" style="vertical-align: middle;" /> $display_txt{'139'}: $pagenumb</span>~;
...
qq~<span class="small" style="float: left; height: 21px; margin: 0px; margin-top: 2px;">~;
            if ( !$iamguest ) {
                $pagetxtindexst .=
                  ...
                  . qq~;action=threadpagedrop"><img src="$imagesdir/index_togl.gif" border="0" alt="$display_txt{'19'}" style="vertical-align: middle;" /></a> $display_txt{'139'}: ~;
            }
            else {
                $pagetxtindexst .=
qq~<img src="$imagesdir/index_togl.gif" border="0" alt="" style="vertical-align: middle;" /> $display_txt{'139'}: ~;
            }
            if ( $startpage > 0 ) {
                $pagetxtindex =
                    qq~<a href="$scripturl?num=$viewnum/~
                  . ( !$ttsreverse ? 0 : $mreplies )
                  . qq~" style="font-weight: normal;">1</a>&nbsp;<a href="javascript:void(0);" onclick="ListPages($mnum);">...</a>&nbsp;~;
            }
            if ( $startpage == $maxmessagedisplay ) {
                $pagetxtindex =
                    qq~<a href="$scripturl?num=$viewnum/~
                  . ( !$ttsreverse ? 0 : $mreplies )
                  . qq~" style="font-weight: normal;">1</a>&nbsp;~;
            }
            for (
                $counter = $startpage ;
                $counter < $endpage ;
                $counter += $maxmessagedisplay
              )
            {
                $pagetxtindex .=
                  $start == $counter
                  ...
                  . qq~" style="font-weight: normal;">$tmpa</a>&nbsp;~;
                $tmpa++;
            }
...
qq~<a href="javascript:void(0);" onclick="ListPages($mnum);">...</a>&nbsp;~;
            ...
            if ( $endpage != $max ) {
                $pageindexadd .=
                  ...
                  . qq~" style="font-weight: normal;">$lastpn</a>~;
            }
...
qq~<span style="float: left; height: 21px; margin: 0; margin-right: 4px;"><a href="$scripturl?num=$viewnum;start=~
              . ( !$ttsreverse ? $start : $mreplies - $start )
              . qq~;action=threadpagetext"><img src="$imagesdir/index_togl.gif" border="0" alt="$display_txt{'19'}" title="$display_txt{'19'}" /></a></span>~;
...
qq~<span id="ViewIndex2" class="droppageindex" style="height: 14px; visibility: hidden">&nbsp;</span>~;
            ...
            $pagedropindexpvbl =
qq~<img src="$imagesdir/index_left0.gif" height="14" width="13" border="0" alt="" style="margin: 0px; display: inline; vertical-align: middle;" />~;
            $pagedropindexnxbl =
qq~<img src="$imagesdir/index_right0.gif" height="14" width="13" border="0" alt="" style="margin: 0px; display: inline; vertical-align: middle;" />~;

            if (   ( !$ttsreverse and $start < $maxmessagedisplay )
                or ( $ttsreverse and $prevpage > $mreplies ) )
            {
                $pagedropindexpv .=
qq~<img src="$imagesdir/index_left0.gif" height="14" width="13" border="0" alt="" style="display: inline; vertical-align: middle;" />~;
            }
            else {
                $pagedropindexpv .=
qq~<img src="$imagesdir/index_left.gif" border="0" height="14" width="13" alt="$pidtxt{'02'}" title="$pidtxt{'02'}" style="display: inline; vertical-align: middle; cursor: pointer;" onclick="location.href=\\'$scripturl?num=$viewnum/$prevpage\\'" ondblclick="location.href=\\'$scripturl?num=$viewnum/~
                  . ( !$ttsreverse ? 0 : $mreplies )
                  . qq~\\'" />~;
            }
            if (   ( !$ttsreverse and $nextpage > $lastptn )
                or ( $ttsreverse and $nextpage < $mreplies - $lastptn ) )
            {
                $pagedropindexnx .=
qq~<img src="$imagesdir/index_right0.gif" border="0" height="14" width="13" alt="" style="display: inline; vertical-align: middle;" />~;
            }
            else {
                $pagedropindexnx .=
qq~<img src="$imagesdir/index_right.gif" height="14" width="13" border="0" alt="$pidtxt{'03'}" title="$pidtxt{'03'}" style="display: inline; vertical-align: middle; cursor: pointer;" onclick="location.href=\\'$scripturl?num=$viewnum/$nextpage\\'" ondblclick="location.href=\\'$scripturl?num=$viewnum/~
                  . ( !$ttsreverse ? $lastptn : $mreplies - $lastptn )
                  . qq~\\'" />~;
            }
      ...
      SelDec('$pagejsindex', '~
              . ( !$ttsreverse ? $tstart : ( $mreplies - $tstart ) ) . qq~');
~;
        }
    }
...
qq~$menusep<a href="javascript:Notify('$scripturl?action=notify3;num=$viewnum/~
              . ( !$ttsreverse ? $start : $mreplies - $start )
              . qq~','$imagesdir')" name="notifylink">$img{'del_notify'}</a>~;
        }
        else {
            $notify =
qq~$menusep<a href="javascript:Notify('$scripturl?action=notify2;num=$viewnum/~
              . ( !$ttsreverse ? $start : $mreplies - $start )
              . qq~','$imagesdir')" name="notifylink">$img{'add_notify'}</a>~;
        }
    }
...
qq~<a name="top"><img src="$imagesdir/$threadclass.gif" style="vertical-align: middle;" alt="" /></a>~;
    $template_sendtopic =
      $sendtopicmail
...
qq~$menusep<a href="$scripturl?action=print;num=$viewnum" target="_blank">$img{'print'}</a>~;
    ...
    foreach (@messages) {
        my (
            ...
            $msnad,             $gtalkad,          $skypead,
            $myspacead,         $facebookad,       $icqad,
            $buddyad,           $addbuddy,         $isbuddy,
            $addbuddylink,      $userOnline,       $signature_hr,
            $lastmodified,      $memberinfo,       $template_postinfo,
            $template_ext_prof, $template_profile, $template_quote,
        ...
        if ( $mfn ne '' ) {
            ...
            foreach ( split( /,/, $mfn ) ) {
                ...
                if ($filesize) {
                    if (   $_ =~ /\.(bmp|jpe|jpg|jpeg|gif|png)$/i
                        && $amdisplaypics == 1 )
                    {
                        $showattach .=
qq~<div class="small" style="float:left; margin:8px;"><a href="$scripturl?action=downloadfile;file=$urlname" target="_blank"><img src="$imagesdir/$attach_gif{$ext}" border="0" align="bottom" alt="" /> $_</a> (~
                          . int( $filesize / 1024 )
                          ...
                          . qq~<img src="$uploadurl/$_" name="attach_img_resize" alt="$_" title="$_" border="0" style="display:none" /></a></div>\n~;
                    }
                    else {
                        $attachment .=
qq~<div class="small"><a href="$scripturl?action=downloadfile;file=$urlname"><img src="$imagesdir/$attach_gif{$ext}" border="0" align="bottom" alt="" /> $_</a> (~
                          . int( $filesize / 1024 )
                    ...
                    $attachment .=
qq~<div class="small"><img src="$imagesdir/$attach_gif{$ext}" border="0" align="bottom" alt="" />  $_ ($fatxt{'1'}~
                      . (
            ...
            $showattachhr =
qq~<hr width="100%" size="1" class="hr" style="margin: 0; margin-top: 5px; margin-bottom: 5px; padding: 0;" />~;
            if ( $showattach && $attachment ) {
                $attachment =~
s/<div class="small">/<div class="small" style="margin:8px;">/g;
            }
        }
...
qq~$menusep<a href="$scripturl?action=modalert;num=$viewnum;title=PostReply;quote=$counter" onclick="return confirm('$display_txt{'alertmod_confirm'}');">$img{'alertmod'}</a>~;
        ...
        if ( $buddyListEnabled && !$iamguest && $musername ne $username ) {
            $isbuddy =
qq~<br /><img src="$imagesdir/buddylist.gif" border="0" align="middle" alt="$display_txt{'isbuddy'}" title="$display_txt{'isbuddy'}" /> <br />$display_txt{'isbuddy'}~;
            $addbuddylink =
...
qq~$menusep<a href="$scripturl?action=imsend;to=$useraccount{$musername}">$img{'message_sm'}</a>~;
            ...
            $tmppostcount =
              &NumberFormat( ${ $uid . $musername }{'postcount'} );
...
qq~<hr width="100%" size="1" class="hr" style="margin: 0; margin-top: 5px; margin-bottom: 5px; padding: 0;" />~
        ...
        {
            $template_email =
              $menusep . &enc_eMail( $img{'email_sm'}, $memailad, '', '' );
            if ($iamadmin) {
                $template_email =~
                  s~title=\\"$img_txt{'69'}\\"~title=\\"$memailad\\"~;
            }
        }
...
qq~$menusep<a href="$scripturl?board=$currentboard;action=modify;message=$counter;thread=$viewnum" onclick="return confirm('$display_txt{'modifyinlocked'}');">$img{'modify'}</a> ~;
        ...
        if ( $mstate !~ /l/i ) {
            if ($replybutton) {
                ...
                $quote_mname =~ s/'/\\'/g;
                $usernamelink =
qq~<a href="javascript:void(AddText('[color=$quoteuser_color]@[/color] [b]$quote_mname\[/b]\\r\\n\\r\\n'))"><img src="$imagesdir/qquname.gif" border="0" alt="$display_txt{'146n'}" title="$display_txt{'146n'}" /></a> $usernamelink~
                ...
                if ( !$movedflag || $iamadmin || $iamgmod || $iammod ) {
                    if ($enable_quickreply) {
                        ...
                        if ($enable_markquote) {
                            $outblock =~
s/(<div)( class="$messageclass" style="float: left; width: 99%; overflow: auto;">)/$1 onmouseup="get_selection($counter);"$2/i;
                            $template_quote =
qq~$menusep<a href="javascript:void(quoteSelection('$quote_mname',$viewnum,$counter,$mdate,''))">$img{'mquote'}</a>~;
                        ...
                        if ($enable_quickjump) {
                            if ( length($postmessage) <= $quick_quotelength ) {
                                ...
                                if ( !$nestedquotes ) {
                                    $quickmessage =~
s~(<(br|p).*?>){0,1}\[quote([^\]]*)\](.*?)\[/quote([^\]]*)\](<(br|p).*?>){0,1}~<br />~ig;
                                }
                                $quickmessage =~ s/<(br|p).*?>/\\r\\n/ig;
                                $quickmessage =~ s/'/\\'/g;
                                $template_quote .=
...
qq~ <input type="checkbox" class="$css" style="border: 0px; visibility: hidden; display: none;" name="del$counter" value="$counter" />~;
        ...
        $msgimg =
qq~<a href="$scripturl?num=$viewnum/$counter#$counter"><img src="$imagesdir/$micon.gif" alt="" border="0" style="vertical-align: middle;" /></a>~;
        $ipimg =
qq~<img src="$imagesdir/ip.gif" alt="" border="0" style="vertical-align: middle;" />~;
        if ($extendedprofiles) {
            require "$sourcedir/ExtendedProfiles.pl";
        ...
        $usernamelink = qq~<a name="new"></a>$usernamelink~
        ...
        $outblock =~
          s/({|<)yabb signature(}|>)/${$uid.$musername}{'signature'}/g;
...
qq~$menusep<a href="$scripturl?action=markunread;thread=$viewnum;board=$currentboard">$img{'markunread'}</a>~;
    ...
    $tabsep =
qq~<img src="$imagesdir/tabsep211.png" border="0" alt="" style="vertical-align: middle;" />~;
    $yynavback =
qq~$tabsep <a href="$scripturl">&#171; $img_txt{'103'}</a> $tabsep $navback $tabsep~;
    ...
    my $curthreadurl =
      ...
      . ( $ttsreverse ? 'up' : 'down' )
      . qq~.gif" border="0" alt="" style="vertical-align: middle;" /> $msubthread</a>~
      : $msubthread;
...
qq~<form name="multidel" action="$scripturl?board=$currentboard;action=multidel;thread=$viewnum/~
      . ( !$ttsreverse ? $start : $mreplies - $start )
      . qq~" method="post" style="display: inline">~;
    $formend = qq~</form>~;
...
<!--~;
    ...
    if ( $replybutton and $enable_quickreply ) {
        $yymain =~
s~(<!-- Threads Admin Button Bar start -->.*?</td>)~$1<td align="right">{yabb forumjump}</td>~s;
...
sub NextPrev {
    ...
    if ($vircurrentboard) {
        fopen( MSGTXT, "$boardsdir/$vircurrentboard.txt" )
          || &fatal_error( "cannot_open", "$boardsdir/$vircurrentboard.txt",
            1 );
    ...
    for ( $i = 0 ; $i < @threadlist ; $i++ ) {
        ...
        if ( $threadstatus =~ /h/i && !$iamadmin && !$iamgmod && !$iammod ) {
            next;
        }
