...
$downloadsplver = 'YaBB 2.6 $Revision: 1.0 $';
...
<br />~;
    ...
    @attachinput =
      grep { $_ =~ /$thread\|.+\|(.+)\|\d+\s+/ && exists $attachinput{$1} }
      <AML>;
    fclose(AML);
    ...
    if ( !$max ) {
        $viewattachments .=
qq~<tr><td align="center" class="windowbg2" colspan="$colspan"><b><i>$fatxt{'48'}</i></b></td></tr>
...
qq~<a href="$scripturl?action=downloadfile;newstart=$lastptn;sort=$sort">$lastpn</a>~;
        ...
        $pageindex =
qq~<div class="small" style="text-align: right;">$fatxt{'64'}: $pageindex</div>~;
        ...
        foreach $row ( splice( @attachments, $newstart, 25 ) ) {
            ...
            $viewattachments .= qq~<tr>~ . (
                ( $iamadmin || $iamgmod )
                ? qq~
            <td class="windowbg2" align="center" valign="middle"><input type="checkbox" name="del_$thread" value="$amfn" /></td>~
                : ''
              )
              . qq~
...
<table border="0" cellspacing="1" cellpadding="8" class="bordercolor" align="center" width="90%">
      ...
      . ( $sort == 7 ? -7 : 7 )
      . qq~';" align="center" class="$class_sortattach" style="border: 0px; border-style: outset; cursor: pointer;"><a href="$scripturl?action=viewdownloads;thread=$thread;sort=~
      . ( $sort == 7 ? -7 : 7 )
      . qq~"><b>$fatxt{'40'}</b></a></td>
            <td onclick="location.href='$scripturl?action=viewdownloads;thread=$thread;sort=~
      . ( $sort == 100 ? -100 : 100 )
      . qq~';" align="center" class="$class_sorttype" style="border: 0px; border-style: outset; cursor: pointer;"><a href="$scripturl?action=viewdownloads;thread=$thread;sort=~
      . ( $sort == 100 ? -100 : 100 )
      . qq~"><b>$fatxt{'40a'}</b></a></td>
            <td onclick="location.href='$scripturl?action=viewdownloads;thread=$thread;sort=~
      . ( $sort == -5 ? 5 : -5 )
      . qq~';" align="center" class="$class_sortsize" style="border: 0px; border-style: outset; cursor: pointer;"><a href="$scripturl?action=viewdownloads;thread=$thread;sort=~
      . ( $sort == -5 ? 5 : -5 )
      . qq~"><b>$fatxt{'41'}</b></a></td>
            <td onclick="location.href='$scripturl?action=viewdownloads;thread=$thread;sort=~
      . ( $sort == -6 ? 6 : -6 )
      . qq~';" align="center" class="$class_sortdate" style="border: 0px; border-style: outset; cursor: pointer;"><a href="$scripturl?action=viewdownloads;thread=$thread;sort=~
      . ( $sort == -6 ? 6 : -6 )
      . qq~"><b>$fatxt{'43'}</b></a></td>
            <td onclick="location.href='$scripturl?action=viewdownloads;thread=$thread;sort=~
      . ( $sort == -8 ? 8 : -8 )
      . qq~';" align="center" class="$class_sorcount" style="border: 0px; border-style: outset; cursor: pointer;"><a href="$scripturl?action=viewdownloads;thread=$thread;sort=~
      . ( $sort == -8 ? 8 : -8 )
      . qq~"><b>$fatxt{'41a'}</b></a></td>
            <td onclick="location.href='$scripturl?action=viewdownloads;thread=$thread;sort=~
      . ( $sort == 1 ? -1 : 1 )
      . qq~';" align="center" class="$class_sortsubj" style="border: 0px; border-style: outset; cursor: pointer;"><a href="$scripturl?action=viewdownloads;thread=$thread;sort=~
      . ( $sort == 1 ? -1 : 1 )
      . qq~"><b>$fatxt{'44'}</b></a></td>
            <td onclick="location.href='$scripturl?action=viewdownloads;thread=$thread;sort=~
      . ( $sort == 3 ? -3 : 3 )
      . qq~';" align="center" class="$class_sortuser" style="border: 0px; border-style: outset; cursor: pointer;"><a href="$scripturl?action=viewdownloads;thread=$thread;sort=~
      . ( $sort == 3 ? -3 : 3 )
...
sub DownloadFileCouter {
    ...
    if ( $guest_media_disallowed && $iamguest ) {
        &fatal_error( "", $maintxt{'40'} );
    }

    if ( !-e "$uploaddir/$dfile" ) {
        &fatal_error( "", "$dfile $maintxt{'23'}" );
    }
    ...
    for ( my $a = 0 ; $a < @attachments ; $a++ ) {
        $attachments[$a] =~
s/(.+\|)(.+)\|(\d+)(\s+)$/ $1 . ($dfile eq $2 ? "$2|" . ($3 + 1) : "$2|$3") . $4 /e;
    }
