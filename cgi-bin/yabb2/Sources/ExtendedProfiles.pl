...
$extendedprofilesplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub ext_get {
    my (
        ...
        @options,   $id,                 $value,     $width,
        $height,    @allowed_extensions, $extension, $match
    ) = ( shift, shift, shift );
    ...
    if ( $no_parse eq "" || $no_parse == 0 ) {
        ...
        if ( $field{'type'} eq "text" ) {
            ...
            if ( $options[0] ne "" && $options[0] != 0 ) {
                $width = " width=\"" . ( $options[0] + 0 ) . "\"";
            }
            else { $width = ""; }
            if ( $options[1] ne "" && $options[1] != 0 ) {
                $height = " height=\"" . ( $options[1] + 0 ) . "\"";
            }
            else { $height = ""; }
            if ( $value !~ m~\Ahttp://~ ) { $value = "http://$value"; }
...
sub ext_get_field {
    ...
    (
        ...
        undef
    ) = split( /\|/, $ext_prof_fields[ $field{'id'} ] );
}
...
sub ext_timeformat {
    my (
        ...
        $newday,         $newday2,      $newmonth,   $newmonth2,
        $newyear,        $newshortyear, $oldmonth,   $oldday,
        $oldyear,        $newweekday,   $newyearday, $newweek,
        $dummy,          $usefullmonth
    );
...
sub ext_viewprofile {
    ...
    if ( $#ext_prof_order > 0 ) {
        $last_field_id = &ext_get_field_id( $ext_prof_order[$#ext_prof_order] );
    }
...
# only print spacer if the previous entry was no spacer of the same type and if this is not the last entry
                if ( ( $previous eq 0 || $field{'comment'} ne "" )
                    && $id ne $last_field_id )
                {
...
sub ext_viewinposts {
    my (
        $pusername, $popup,      @ext_profile, $id,
        $output,    $fieldname,  @options,     $value,
        $previous,  $pre_output, $visible,     $users,
        $groups,    $displayfieldname
    ) = ( shift, shift );
...

{
    ...
    sub ext_viewinposts_popup {
        ...
        if ( $output ne "" ) {
            $link =~
s~<a ~<a onmouseover="document.getElementById('ext_$ext_usercount').style.visibility = 'visible'" onmouseout="document.getElementById('ext_$ext_usercount').style.visibility = 'hidden'" ~ig;
            $output =
...
sub ext_memberlist_tableheader {
 ...
 # make sure the field is visible and the user allowed to view the current field
        ...
        {
            $output .=
              qq~<td class="catbg" align="center">$field{'name'}</td>\n~;
        }
    }
...
sub ext_memberlist_tds {
    my (
        $pusername, $usergroup, @ext_profile, $id,     $output,
...
#} else {
            ...
            if ( $field{'type'} eq "email" ) {
                if ( $value ne "" ) {
                    $value = &enc_eMail( $img_txt{'69'}, $value, '', '' );
                }
            }
...
sub ext_gen_editfield {
    my (
        ...
        @options,    $selected,    $count,       $required_prefix,
        $dayormonth, $dayormonthd, $dayormonthm, $value,
        $template1,  $template2
    ) = ( shift, shift );
    ...
    $template1 =
qq~<tr class="windowbg"><td align="left" valign="top"><label for=""><b>$field{'name'}: </b><br /><span class="small">$field{'comment'}</span></label></td><td align="left">~;
    ...
    if ( $field{'type'} eq "text" ) {
      ...
      <br /><span class="small">$lang_ext{'max_chars1'}$options[0]$lang_ext{'max_chars2'} <input value="$options[0]" size="~
              . length( $options[0] )
              . qq~" name="ext_$id\_msgCL" id="ext_$id\_msgCL" class="windowbg" style="border: 0px; padding: 1px; font-size: 11px;" readonly="readonly" /></span>
      ...
      //-->
        ...
        if   ( $options[1] ne "" ) { $options[1] = qq~ rows="$options[1]"~; }
        else                       { $options[1] = qq~ rows="4"~; }
        if   ( $options[2] ne "" ) { $options[2] = qq~ cols="$options[2]"~; }
        else                       { $options[2] = qq~ cols="50"~; }
        $value =~ s/<br(?: ?\/)?>/\n/g;
        $output .=
            $template1
...
sub ext_editprofile {
    my (
        $pusername, $part,    $usergroup, $id, $output,
...
sub ext_validate_submition {
    my (
        $username, $pusername, $usergroup, %newprofile, @oldprofile,
        $output,   $key,       $value,     $id,         @options
...
# if ($options[3] ne "" && $value eq "") { $value = $options[3]; $newprofile{'ext_'.$id} = $value; }
                ...
                if ( $options[2] == 1 && $value !~ /[0-9\.,]+/ && $value ne "" )
                {
                    $output .=
                        $field{'name'} . ": "
                ...
                || $field{'type'} eq "radiobuttons" )
            {
                ...
                $newprofile{ 'ext_' . $id } =
                    $newprofile{ 'ext_' . $id . '_month' } . "\/"
                  . $newprofile{ 'ext_' . $id . '_day' } . "\/"
                  . $newprofile{ 'ext_' . $id . '_year' };
                if ( $newprofile{ 'ext_' . $id } !~ /^\d\d\/\d\d\/\d\d\d\d$/ ) {
                    $newprofile{ 'ext_' . $id } = "";
                }
                next;
                ...
                if   ( $value ne "" ) { $newprofile{ 'ext_' . $id } = 1; }
...
# check if required fields are filled and add missing fields to $newprofile, just to be on the safe side
    ...
    foreach (@ext_prof_fields) {
        ...
        if ( defined $newprofile{ 'ext_' . $id } && $field{'type'} eq "select" )
        {
            ...
            @options = split( /\^/, $field{'options'} );
            if ( $options[ $newprofile{ 'ext_' . $id } ] eq " " ) {
                $newprofile{ 'ext_' . $id } = "";
            }
        }
...
#           if (!defined $newprofile{'ext_'.$id} || $field{'active'} == 0) { $newprofile{'ext_'.$id} = $value; }
        if (   $field{'required_on_reg'} == 1
            && $newprofile{ 'ext_' . $id } eq ""
            && $action eq "register2" )
        {
            $output .=
              $field{'name'} . ": " . $lang_ext{'required'} . "<br />\n";
        }
...
~;
    if ( !@ext_prof_order ) {
        $yymain .=
qq~<td class="windowbg2" colspan="4" align="center"><br /><i>$lang_ext{'no_additional_fields_set'}</i><br /><br /></td>~;
    }
    else {
        foreach $fieldname (@ext_prof_order) {
            $id = &ext_get_field_id($fieldname);
...
qq~<option value="NoPost{$groupid}"$groupcheck{'NoPost{'.$groupid.'}'}>~
          . ( split( /\|/, ( split( /\|/, $NoPost{$groupid} ) )[0] ) )[0]
          . qq~</option>\n~;
    }
    foreach ( sort { $b <=> $a } keys %Post ) {
        $groupid = $_;
...
sub ext_admin_edit {
    ...
    if ( $FORM{'apply'} ne "" ) {
        ...
        $oldname = $_[0];
        $fields[ $FORM{'id'} ] =
"$name|$type|$_[2]|$active|$_[4]|$_[5]|$_[6]|$_[7]|$_[8]|$_[9]|$_[10]|$_[11]|$_[12]|$_[13]|$_[14]|$_[15]|$_[16]|$_[17]|$_[18]|$_[19]|$_[20]|$_[21]";
        ...
        $editable_check[ $field{'editable_by_user'} ] =
          " selected=\"selected\"";
        $yymain .= qq~
...
qq~<input name="radiounselect" id="radiounselect" type="checkbox" value="1"$radiounselect />~
            ...
            else { $check2 = ""; $check1 = " checked=\"checked\""; }
...
qq~<select name="editable_by_user" id="editable_by_user" size="1">\n~
      ...
      <tr>
        ...
        if ( $field{'type'} eq "spacer" ) {
            $yymain .=
              qq~<input name="editable_by_user" type="hidden" value="1" />
                  ~;
        }
        $yymain .=
...
"$FORM{'limit_len'}^$FORM{'width'}^$FORM{'is_numeric'}^$FORM{'default'}^$FORM{'ubbc'}";
        ...
        if ( $FORM{'visible_in_editprofile'} eq "" ) {
