...
$favoritesplver = 'YaBB 2.6 $Revision: 1.0 $';
...
# grab all relevant info on the favorite thread for this user and check access to them
    ...
    foreach ( sort(@favboards) ) {
        ...
        next
          if !$iamadmin
              && &AccessCheck( $loadboard, '',
                  ( split( /\|/, $board{$loadboard} ) )[1] ) ne "granted";

        next
          if !$iamadmin
          && !&CatAccess(
            ( split( /\|/, $catinfo{"${$uid.$loadboard}{'cat'}"} ) )[1] );

        fopen( BRDTXT, "$boardsdir/$loadboard.txt" )
          || &fatal_error( "cannot_open", "$boardsdir/$currentboard.txt", 1 );
        foreach (<BRDTXT>) {
            if ( ( split( /\|/, $_, 2 ) )[0] eq $loadfav ) {
                push( @threads, $_ );
            }
        }
        fclose(BRDTXT);
        ...
        if ( !$iamguest && $max_log_days_old ) {
                ...
                || ( $dlp > $dmax && $dlp < $mdate ) )
            {
                if ( ${$mnum}{'board'} eq $annboard ) {
                    $new =
qq~<a href="$scripturl?virboard=$currentboard;num=$mnum/new"><img src="$imagesdir/new.gif" alt="$messageindex_txt{'302'}" title="$messageindex_txt{'302'}" border="0"/></a>~;
                }
                else {
                    $new =
qq~<a href="$scripturl?num=$mnum/new"><img src="$imagesdir/new.gif" alt="$messageindex_txt{'302'}" title="$messageindex_txt{'302'}" border="0"/></a>~;
                }
...
qq~<br /><span class="small">&#171; $messageindex_txt{'139'} $pages &#187;</span>~;
            ...
            && !( -e "$memberdir/$lastposter.vars" ) )
        {
            ...
            unless (
                (
                       $lastposter eq $messageindex_txt{'470'}
                    || $lastposter eq $messageindex_txt{'470a'}
                )
                && -e "$memberdir/$lastposter.vars"
              )
            {
                ...
                if ( ${ $uid . $lastposter }{'realname'} ) {
                    $lastposter =
qq~<a href="$scripturl?action=viewprofile;username=$lastposter">${$uid.$lastposter}{'realname'}</a>~;
                }
            }
        }
...
qq~$messageindex_txt{'75'}<br />$messageindex_txt{'76'} $curfav $messageindex_txt{'77'} $maxfavs $messageindex_txt{'78'}~;
    ...
    $bdpic =
qq~ <img src="$imagesdir/favboards.gif" alt="$img_txt{'70'}" title="$img_txt{'70'}" border="0" align="middle" /> ~;
...
sub IsFav {
    ...
    if ( @oldfav < ( $maxfavs || 10 ) ) {
        $button =
qq~$menusep<a href="javascript:AddRemFav('$scripturl?action=addfav;fav=$favo;start=$goto','$imagesdir')" name="favlink">$img{'addfav'}</a>~;
        $nofav = 1;
    }
    else { $nofav = 2; }
    ...
    foreach (@oldfav) {
        if ( $favo eq $_ ) {
            $button =
qq~$menusep<a href="javascript:AddRemFav('$scripturl?action=remfav;fav=$favo;start=$goto','$imagesdir')" name="favlink">$img{'remfav'}</a>~;
            $nofav = 0;
        }
    }
