...
$guardianplver = 'YaBB 2.6 $Revision: 1.0 $';
...
qq~$guardian_txt{'abuse_user'}: $username -> (${$uid.$username}{'realname'})\n~;
        ...
        while ( ( $key, $secvalue ) = each(%FORM) ) {
            $secvalue = lc($secvalue);
            $secvalue =~ s/\[code.*?\/code\]//gs
              if $key eq 'message' and $action =~ /^(post|modify|imsend)2$/;
            &str_replace( "%3c", "<", $secvalue );
            &str_replace( "%3e", ">", $secvalue );
            if (   ( $secvalue =~ m/<[^>]script*\"?[^>]*>/ )
                || ( $secvalue =~ m/<[^>]style*\"?[^>]*>/ ) )
...
sub get_ip {
    ...
    if ( $client_ip && $client_ip !~ m/empty/ && $client_ip !~ m/unknown/ ) {
        ...
        && $x_forwarded !~ m/empty/
        && $x_forwarded !~ m/unknown/ )
    {
        return $x_forwarded;
    }
    elsif ($x_ip_client
        && $x_ip_client !~ m/empty/
        && $x_ip_client !~ m/unknown/ )
    {
        return $x_ip_client;
    }
