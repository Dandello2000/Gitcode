...
$helpcentreplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub MainHelp {
    ...
    while ( ${ SectionSub . $i } ) {
        ...
        $message =~
s~\[yabbc\](.*?)\[/yabbc\]~my($text) = $1; &ToHTML($text); &DoUBBCTo($text);~sge;
        &wrap2;
...

{
    ...
    sub codehlp {
        ...
        $hcode =~
          s~(&#91\;.+?&#93\;)~<span style="color: #ff0000;">$1</span>~isg;
        $hcode =~
          s~(&#91\;&#47\;.+?&#93\;)~<span style="color: #ff0000;">$1</span>~isg;
        return $hcode;
    }
}
...
sub DoContents {
    ...
    $Contents .=
      qq~<ul style="list-style: none; margin: 0; padding: 2px; border: none;">~;
    $i = 1;
    while ( ${ SectionSub . $i } ) {
