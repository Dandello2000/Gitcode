...
$mediacenterplver = 'YaBB 2.6 $Revision: 1.0 $';
...
qq~ $maintxt{'42'} <a href="$scripturl?action=register">$img{'register'}</a> !!~;
            ...
            m/(\.wmv|\.wpl|\.asf|\.avi|\.mpg|\.mpeg|\.divx|\.xdiv)$/i )
        {
            ...
            m/(\.wma|\.wax|\.asx|\.mp3|\.mid|\.wav|\.kar|\.rmi)$/i )
        {
            if ( $player_version == 6 ) {
                $video = $embed_wma6;
            }
