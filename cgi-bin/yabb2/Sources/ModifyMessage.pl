...
$modifymessageplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub ModifyMessage {
    ...
    my (
        $mnum,      $msub,  $mname,  $memail,   $mdate,   $mreplies,
        $musername, $micon, $mstate, $msubject, $mattach, $mip,
        $mmessage,  $mns,   $mlm,    $mlmb
    );
    ...
    if ( $postid eq "Poll" ) {
        ...
        unless ( $poll_uname eq $username || $iammod || $iamadmin || $iamgmod )
        {
        ...
        (
            ...
            $mlm,   $mlmb,    $mfn
        ) = split( /\|/, ${ $thread_arrayref{$threadid} }[$postid] );
        chomp $mfn;
...
sub ModifyMessage2 {
    ...
    my (
        ...
        $tusername,  $ticon,   $tstate,          @threads,
        $tmpa,       $tmpb,    $newlastposttime, $newlastposter,
        $lastpostid, $views,   $name,            $email,
        $subject,    $message, $ns,
    );
...
qq~$threadid|$postid|$subject|$mname|$currentboard|$filesizekb|$date|$fixfile|0\n~
    ...
    truncate ATM, 0;
    seek ATM, 0, 0;
    print ATM sort { ( split /\|/, $a )[6] <=> ( split /\|/, $b )[6] }
      @attachments;
    fclose(ATM);
...
# Save the current board. icon, status or subject may have changed -> update board info
        ...
        for ( my $a = 0 ; $a < @board ; $a++ ) {
            if ( $board[$a] =~ m~\A$threadid\|~o ) {
                $board[$a] = "$yyThreadLine\n";
                last;
            }
        }
        truncate BOARD, 0;
...
sub MultiDel {    # deletes singel- or multi-Posts
    ...
    for ( $count = $#messages ; $count >= 0 ; $count-- ) {
        if ( $FORM{"del$count"} ne '' ) {
            ...
            unless ( ${ $uid . $currentboard }{'zero'}
                || $musername  eq 'Guest'
                || $message[6] eq 'no_postcount' )
            {
                ...
                &ManageMemberinfo( "update", $musername, '', '', $grp_after,
                    ${ $uid . $musername }{'postcount'} );
