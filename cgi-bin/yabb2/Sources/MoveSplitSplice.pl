...
$movesplitspliceplver = 'YaBB 2.6 $Revision: 1.0 $';
...
<br />~;
    ...
    unless ( exists $FORM{'oldposts'} ) {
        $FORM{'oldposts'} = $INFO{'oldposts'};
    }
    ...
    for ( $counter = 0 ; $counter < @messages ; $counter++ ) {
        ...
        $messages[$counter] =
          ...
          . (
            $FORM{'oldposts'} =~ /\b$counter\b/ ? q~selected="selected"~ : '' )
          . qq~>~
    ...
    for ( $counter = 0 ; $counter < @leaveopts ; $counter++ ) {
        $leavelist .=
            qq~<option value="$counter" ~
          . ( $FORM{'leave'} == $counter ? q~selected="selected"~ : '' )
    ...
    foreach (@categoryorder) {
        ...
        $catlist .=
            qq~<option value="$_" ~
          . ( $newcat eq $_ ? q~selected="selected"~ : '' )
          . qq~>$catname</option>\n~;
    }
    ...
    foreach ( split( /,/, $cat{$newcat} ) ) {
        ...
        $boardlist .=
            qq~<option value="$_" ~
          . ( $newboard eq $_ ? q~selected="selected"~ : '' )
          . qq~>$boardname</option>\n~;
    }
    ...
    if ( $cat{$newcat} =~ /\b$newboard\b/ ) {
        ...
        foreach (@threads) {
            ...
            $threadlist .=
              ...
              . ( $newthread eq $threadid ? q~selected="selected"~ : '' )
              . qq~>$message</option>\n~;
        }
        ...
        if ( $FORM{'newthread'} ne "new" ) {
            ...
            {
                $positionlist =~
                  s/(value="$FORM{'position'}")/$1 selected="selected"/;
            }
        }
    }
...
sub Split_Splice_2 {
    ...
    if ( ( split( /\, /, $movingposts, 2 ) )[0] eq "all" ) {
        @postnum = ( 0 .. $#curthread );
    }
    else {
        @postnum = sort { $a <=> $b } split( /\, /, $movingposts );
...
# curthread
    ...
    for ( $i = 0 ; $i < @curmessindex ; $i++ ) {
        ...
        ) = split( /\|/, $curmessindex[$i] );
        $boardlog = 0
          if $mdate > $yyuserlog{$curboard};    # For: Mark boards as read
        if ( $mnum == $curthreadid ) {
            chomp $mstate;
            if ( $#postnum == $#curthread && $leavemess != 1 )
            {                                   # thread was moved
...
qq~[m by=$hidename destboard=$newboard dest=$newthreadid]: '$msub'~;
      ...
      # thread status - (a)nnoumcement, (h)idden, (l)ocked, (m)oved and (s)ticky
                $old_mstate = $mstate;
                if ( $curboard eq $annboard && $mstate !~ /a/i ) {
                    $mstate .= "a";
                }
    ...
    if ( $curboard ne $newboard ) {
        ...
        if ( $FORM{'newthread'} eq 'new' ) {
            ...
            foreach (@newmessindex) {
                $boardlog = 0
                  if ( split( /\|/, $_, 6 ) )[4] > $yyuserlog{$newboard};
                last if !$boardlog;
            }
...
qq~$newthreadid|$msub|$mname|$memail|${$newthreadid}{'lastpostdate'}|${$newthreadid}{'replies'}|$musername|$micon|$old_mstate\n~;
            ...
            for ( $i = 0 ; $i < @newmessindex ; $i++ ) {
                my (
                    $mnum,     $msub,      $mname, $memail, $mdate,
                    $mreplies, $musername, $micon, $mstate
                ) = split( /\|/, $newmessindex[$i] );
                $boardlog = 0
                  if $mdate > $yyuserlog{$newboard};  # For: Mark boards as read
                if ( $mnum == $newthreadid ) {
                    chomp $mstate;
                    if ( $FORM{'position'} eq 'begin' ) {
                        (
...
#&BoardTotals("load", $curboard); - Load this at top now to detect if newest board post is being moved - Unilat
    if ( ${ $BoardTotals{$curthreadid} }[6] =~ /m/ ) {    # Moved-Info thread
          ...
          )
        {
            ...
            ${ $uid . $curboard }{'lastposttime'} && $curboard eq $newboard )
        {
            ${ $uid . $curboard }{'lastposttime'} =
              ${ $BoardTotals{$newthreadid} }[0];
            ${ $uid . $curboard }{'lastposter'} =
...
qq~$newthreadid|$mreplies|$msub|$mname|$newboard|$asize|$mdate|$_|~
    ...
    if ( $#postnum == $#curthread ) {
        if ( -e "$datadir/$curthreadid.poll" ) {
            rename( "$datadir/$curthreadid.poll",
        ...
        if ( -e "$datadir/$curthreadid.mail" ) {
            rename( "$datadir/$curthreadid.mail",
