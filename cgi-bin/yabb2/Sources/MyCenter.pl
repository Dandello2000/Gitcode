...
$mycenterplver = 'YaBB 2.6 $Revision: 1.0 $';
...
qq~&rsaquo; <a href="$scripturl?action=mycenter" class="nav">$img_txt{'mycenter'}</a> &rsaquo; $mctitle~;
    ...
    $mycenter_template =~
      s/({|<)yabb mcglobformend(}|>)/ ($MCGlobalFormStart ? "<\/form>" : "") /e;
...
{    # look for $user.$pmFile, find $id message and check for $messageFlag
    ...
    if ( %{ 'MF' . $user . $pmFile } ) {
        if ( exists ${ 'MF' . $user . $pmFile }{$id}
            && ${ 'MF' . $user . $pmFile }{$id} =~ /$messageFlag/i )
        {
            $messageFoundFlag = 1;
        }
    }
        ...
        foreach (@userMessages) {
            ...
            if ( $uMessageId == $id && $uMessageFlags =~ /$messageFlag/i ) {
                $messageFoundFlag = 1;
            }
        }
    }
...
{ # look for $user.$pmFile, find $id message and check for $messageFlag. change to $newMessageFlag
    ...
    if (
        (
            ...
            || ( $messageFlag ne ''
                && ${ 'MF' . $user . $pmFile }{$id} =~ /$messageFlag/ )
            || ( $messageFlag eq ''
                && !${ 'MF' . $user . $pmFile }{$id} =~ /$newMessageFlag/ )
        )
      ...
      )
    {
        ...
        foreach my $userMessage (@userFile) {
            ...
            if ( $uMessageId == $id ) {
                $uMessageFlags =~ s/$newMessageFlag//gi
                  if $newMessageFlag ne '';
...
{    # update .ims file for user: &updateIMS(<user>,<PM msgid>,[target/action])
    ...
    if ( $target eq 'messagein' ) {
        ...
        if ( &updateMessageFlag( $user, $id, 'msg', 'u', '' ) ) {
            ${$user}{'PMimnewcount'}--;
        }
        else { return; }
        ...
        foreach (@messages) {
            ...
            if ( !exists $FORM{ "message" . $m[0] } ) {
                ...
                if ( $INFO{'caller'} == 3 ) {
                    ...
                    ${$username}{'PMbcRead'} =~ s/$m[0]\b//g
                      unless ${$username}{'PMbcRead'} =~ s/\b$m[0]$//;
                }
            }
        }
        fclose(USRFILE);
...
sub CreateUserDisplayLine {
    ...
    if ( $yyUDLoaded{$usrname} ) {
          ...
          )
        {
            ...
            unless ( $INFO{'caller'} == 5 && $mstatus eq 'g' ) {
...
qq~$menusep<a href="mailto:${$uid.$usrname}{'email'}">$img{'email_sm'}</a>~;
            ...
            ) ? qq~$menusep${$uid.$usrname}{'weburl'}~ : '';
            my $aimad =
              ${ $uid . $usrname }{'aim'}
              ? qq~$menusep${$uid.$usrname}{'aim'}~
              : '';
            my $icqad =
              ${ $uid . $usrname }{'icq'}
              ? qq~$menusep${$uid.$usrname}{'icq'}~
              : '';
            my $yimad =
              ${ $uid . $usrname }{'yim'}
              ? qq~$menusep${$uid.$usrname}{'yim'}~
              : '';
            my $msnad =
              ${ $uid . $usrname }{'msn'}
              ? qq~$menusep${$uid.$usrname}{'msn'}~
              : '';
            my $gtalkad =
              ${ $uid . $usrname }{'gtalk'}
              ? qq~$menusep${$uid.$usrname}{'gtalk'}~
              : '';
            my $skypead =
              ${ $uid . $usrname }{'skype'}
              ? qq~$menusep${$uid.$usrname}{'skype'}~
              : '';
            my $myspacead =
              ${ $uid . $usrname }{'myspace'}
              ? qq~$menusep${$uid.$usrname}{'myspace'}~
              : '';
            my $facebookad =
              ${ $uid . $usrname }{'facebook'}
              ? qq~$menusep${$uid.$usrname}{'facebook'}~
              : '';
...
qq~[quote author=$cloakedAuthor link=impost date=$mdate\]$message\[/quote\]\n~;
            ...
            if ( $message =~ /\#nosmileys/isg ) {
                $message =~ s/\#nosmileys//isg;
                $nscheck = "checked";
            }
        }
...
sub PmPageindex {
    ...
    $yySetLocation =
      ...
      . ( $INFO{'viewfolder'} ? ";viewfolder=$INFO{'viewfolder'}" : "" );
    &redirectexit;
}
...
qq~<form action="$scripturl?action=$destination" method="post" name="postmodify" id="postmodify" enctype="application/x-www-form-urlencoded" onsubmit="~;
            ...
            $MCGlobalFormStart .=
qq~if(!checkForm(this)) { return false; } else { return submitproc(); }">~;
...
qq~<form action="$scripturl?action=$destination" method="post" name="postmodify" id="postmodify" enctype="application/x-www-form-urlencoded">~;
    ...
    if ( $view eq 'mycenter' ) {
        ...
        $myprofileblock =~
          s/({|<)yabb userpic(}|>)/${$uid.$username}{'userpic'}/g;
        $myprofileblock =~
          s/({|<)yabb usertext(}|>)/${$uid.$username}{'usertext'}/g;
        $myprofileblock =~ s/({|<)yabb postinfo(}|>)/$template_postinfo/g;
        $myprofileblock =~ s/({|<)yabb location(}|>)/$userlocation/g;
        $myprofileblock =~
          s/({|<)yabb gender(}|>)/${$uid.$username}{'gender'}/g;
        ################################
...
qq~$mycenter_txt{'buddylisttitle'}:<br />$buddiesCurrentStatus~;
        ...
        || ( $PM_level == 3 && ( $iamadmin || $iamgmod ) ) )
    {
      ...
      <div id="cont_pm" style="display: $display_pm">
        ...
        if ( ( $PMenableBm_level == 1 && ( $iamadmin || $iamgmod || $iammod ) )
...
qq~<span class="NewLinks">, <a href="$scripturl?action=imshow;caller=1;id=-1">${$username}{'PMimnewcount'} $inmes_txt{'new'}</a></span>~;
        ...
        if ( $enable_storefolders > 0 ) {
            ...
            if ( ${$username}{'PMfolders'} ) {
                my $x = 2;
                foreach
                  my $storefolder ( split( /\|/, ${$username}{'PMfolders'} ) )
                {
...
sub drawPMView {
    ...
    if ( ( $#dimmessages >= $maxmessagedisplay || $INFO{'start'} =~ /all/ )
        && $action ne 'imstorage' )
    {
        $MCContent .= qq~
...
##########  top of messages list ##########
            ...
            my (
                ...
                $mreplyno,     $mips,         $messageStatus,
                $messageFlags, $storeFolder,  $messageAttachment
            ) = split( /\|/, $dimmessages[$counter] );
            ...
            my $messIcon =
qq~<img src="$imagesdir/$messIconName.gif" name="icons" border="0" hspace="15" alt="$im_message_status{$messIconName}" title="$im_message_status{$messIconName}" style="vertical-align: middle;" />~;

            my ( $hasMultiRecs, $multiRecs );
            if ( $musernameto =~ /,/ || $musernamecc || $musernamebcc ) {
                $hasMultiRecs = 1;
            }
                    ...
                    if ( $messageStatus !~ /b/ ) {
                        ...
                        foreach my $muser ( split( /\,/, $musernameto ) ) {
                            ...
                            if ( $usernameto && $switchComma == 0 ) {
                                ...
                                $usernameto =
qq~<a href="$scripturl?action=viewprofile;username=$useraccount{$muser}">${$uid.$muser}{'realname'}</a>~;
                            }
                        }
                        if ($musernamecc) {
                            ## check each to see if they read the message
                    ...
                    if ( $messageStatus !~ /b/ ) {
                        $userToMessRead = &checkIMS( $musernameto, $messageid,
                            'messageopened' );
                        if ( !$yyUDLoaded{$musernameto} ) {
                            &LoadUser($musernameto);
                        }
                        $usernameto =
qq~<a href="$scripturl?action=viewprofile;username=$useraccount{$musernameto}">${$uid.$musernameto}{'realname'}</a>~;
                    }
                    else {
            ...
            if ( $action ne 'imstorage' && $action ne 'imdraft' && !$viewBMess )
            {
                ...
                if ( $action eq 'im' ) {
                    ...
                    if ( !$imOpened && !$hasMultiRecs ) {
                        $messageIcon =
qq~<img src="$imagesdir/imclose.gif" border="0" alt="$inmes_imtxt{'innotread'}" title="$inmes_imtxt{'innotread'}" style="vertical-align: middle;" />~;
                    }
                    ## replied to
                    elsif ( $imRepliedTo && !$hasMultiRecs ) {
                        $messageIcon =
qq~<img src="$imagesdir/answered.gif" border="0" alt="$inmes_imtxt{'08'}" title="$inmes_imtxt{'08'}" style="vertical-align: middle;" />~;
                    }
                    ## opened
                    elsif ( $imOpened && !$hasMultiRecs ) {
                        $messageIcon =
qq~<img src="$imagesdir/imopen.gif" border="0" alt="$inmes_imtxt{'inread'}" title="$inmes_imtxt{'inread'}" style="vertical-align: middle;" />~;
                    }
                    ## not opened multi
                    elsif ( !$imOpened && $hasMultiRecs ) {
                        $messageIcon =
qq~<img src="$imagesdir/imclose2.gif" border="0" alt="$inmes_imtxt{'inread'}" title="$inmes_imtxt{'inread'}" style="vertical-align: middle;" />~;
                    }
                    ## opened multi
                    elsif ( $imOpened && $hasMultiRecs ) {
                        $messageIcon =
qq~<img src="$imagesdir/imopen2.gif" border="0" alt="$inmes_imtxt{'inread'}" title="$inmes_imtxt{'inread'}" style="vertical-align: middle;" />~;
                    }
                }
                ...
                ##  outbox
                    ...
                    if ( !$imOpened && !$hasMultiRecs ) {
                        ...
                        {
                            $messageIcon =
qq~<img src="$imagesdir/imclose.gif" border="0" alt="$inmes_imtxt{'outnotread'}" title="$inmes_imtxt{'outnotread'}" style="vertical-align: middle;" />~;
                            $callBack =
...
qq~<img src="$imagesdir/imclose.gif" border="0" alt="$inmes_imtxt{'outnotread'}" title="$inmes_imtxt{'outnotread'}" style="vertical-align: middle;" />~;
                    ...
                    ## opened
                        ...
                        $messageIcon =
                          $messageFlags =~ /c/i
                          ? qq~<img src="$imagesdir/imcallback.gif" border="0" alt="$inmes_imtxt{'callback'}" title="$inmes_imtxt{'callback'}" style="vertical-align: middle;" />~
                          : qq~<img src="$imagesdir/imopen.gif" border="0" alt="$inmes_imtxt{'outread'}" title="$inmes_imtxt{'outread'}" style="vertical-align: middle;" />~;
                    }
                    ...
                    if ($hasMultiRecs) {
                        ...
                        if ( !$countread ) {
                            $messageIcon =
qq~<img src="$imagesdir/imclose2.gif" border="0" alt="$inmes_imtxt{'outmultinotread'}" title="$inmes_imtxt{'outmultinotread'}" style="vertical-align: middle;" />~;
                            $callBack =
qq~<span class="small"><a href="$scripturl?action=imcb;rid=$messageid;receiver=~
                            ...
                            $messageIcon =
                              $messageFlags =~ /c/i
                              ? qq~<img src="$imagesdir/imcallback2.gif" border="0" alt="$inmes_imtxt{'outmulticallback'}" title="$inmes_imtxt{'outmulticallback'}" style="vertical-align: middle;" />~
                              : qq~<img src="$imagesdir/imopen2.gif" border="0" alt="$inmes_imtxt{'outmultiread'}" title="$inmes_imtxt{'outmultiread'}" style="vertical-align: middle;" />~;
                        }
                        else {
                            $messageIcon =
                              $messageFlags =~ /c/i
                              ? qq~<img src="$imagesdir/imcallback3.gif" border="0" alt="$inmes_imtxt{'outsomemulticallback'}" title="$inmes_imtxt{'outsomemulticallback'}" style="vertical-align: middle;" />~
                              : qq~<img src="$imagesdir/imopen3.gif" border="0" alt="$inmes_imtxt{'outmultisomeread'}" title="$inmes_imtxt{'outmultisomeread'}" style="vertical-align: middle;" />~;
                            $callBack =
qq~<span class="small"><a href="$scripturl?action=imshow;id=$messageid;caller=2">$inmes_imtxt{'multicallback'}</a> | </span>~;
        ...
        <tr>
          <td class="titlebg"  width="100%" colspan="3"><span class="imgtitlebg">$im_sorted{'standart'}</span></td>
            ...
            if (   $stkmess
                ...
                && $stkDateSet
                && $viewBMess
                && ( $messageStatus =~ /g/ || $messageStatus =~ /a/ ) )
            {
                ## sticky messages
                $stkDateSet = 0;
                $MCContent .= qq~
        ...
        <tr>
          <td class="titlebg"  width="100%" colspan="3"><span class="imgtitlebg">$im_sorted{$uselegend}</span></td>
            ...
            {
                $BCnew =
qq~&nbsp;<img src="$imagesdir/new.gif" alt="" border="0" style="vertical-align: middle;" />~;
            }
...
qq~$guestName<br />(<a href="mailto:$guestEmail">$guestEmail</a>)~;
                    ...
                    foreach $uname ( split( /,/, $uname ) ) {
                        ...
                        push(
                            ...
                            (
                                ${ $uid . $uname }{'realname'}
                                ? qq~<a href="$scripturl?action=viewprofile;username=$useraccount{$uname}">${$uid.$uname}{'realname'}</a>~
                                : (
                ...
                $MCContent .=
                  join( ', ', @usernameto );    # [outbox / storage out]
...
qq~($inmes_imtxt{'13'} $messageCounter $inmes_imtxt{'01'} $numstore $inmes_imtxt{'19'} $inmes_txt{'storage'} $inmes_txt{'folder'})~;
            ...
            if ( $imrest <= 0 ) { $dorest = ''; }
                ...
                $dorest =
qq~<img src="$imagesdir/usageempty.gif" height="8" width="$imrest" align="middle" alt="" />~;
            }
            $imbargfx =
qq~$inmes_imtxt{'67'}:&nbsp;<img src="$imagesdir/usage.gif" align="middle" alt="" /><img src="$imagesdir/usagebar.gif" height="8" width="$imbar" align="middle" alt="" />$dorest<img src="$imagesdir/usage.gif" align="middle" alt="" />&nbsp;$impercent&nbsp;%&nbsp;<br />~;
        }
        else {
            $intext   = qq~&nbsp;~;
            $imbargfx = qq~&nbsp;~;
        ...
        <tr>
          <td class="titlebg" colspan="3" align="right" height="21" >
            ...
            if (
                ...
                && !( $action eq 'imstorage' && $INFO{'viewfolder'} eq '' )
              )
            {
                $MCContent .= qq~
        <tr>
...
qq~<img src="$imagesdir/lockmail.gif" alt="$mycenter_txt{'hiddenemail'}" title="$mycenter_txt{'hiddenemail'}" />~;
                ...
                $buddyemail =
qq~<a href="mailto:${$uid.$buddyname}{'email'}"><img src="$imagesdir/email.gif" border="0" alt="$profile_txt{'889'} ${$uid.$buddyname}{'email'}" title="$profile_txt{'889'} ${$uid.$buddyname}{'email'}" /></a>~;
            }
            ...
            {
                $buddypm =
qq~<a href="$scripturl?action=imsend;to=$useraccount{$buddyname}"><img src="$imagesdir/imclose.gif" border="0" alt="$profile_txt{'688'} $buddyrealname" title="$profile_txt{'688'} $buddyrealname" /></a>~;
            }

            if ( ${ $uid . $buddyname }{'weburl'} ) {
                $buddywww =
qq~<a href="${$uid.$buddyname}{'weburl'}" target="_blank"><img src="$imagesdir/www.gif" border="0" alt="${$uid.$buddyname}{'webtitle'}" title="${$uid.$buddyname}{'webtitle'}" /></a>~;
            }
        ...
        $buddiesCurrentStatus .=
qq~<tr class="$css"><td align="left">$usernamelink</td><td align="center">$online</td><td align="center">$buddypm</td><td align="center">$buddyemail</td><td align="center">$buddywww</td></tr>~;
        $counter++;
    }
    undef %UserPM_Level;
...
sub mcMenu {
    ...
    my $tabsep =
qq~<img src="$imagesdir/tabsep211.png" border="0" alt="" style="float: left; vertical-align: middle;" />~;
    my $tabfill =
qq~<img src="$imagesdir/tabfill.gif" border="0" alt="" style="vertical-align: middle;" />~;
