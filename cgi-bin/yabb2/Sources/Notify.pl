...
$notifyplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub ManageBoardNotify {
    ...
    if ( $todo eq "add" ) {
        ...
        foreach my $u ( split( ',', $user ) ) {
            delete $theboard{$u};
            &LoadUser($u);
            foreach ( split( /,/, ${ $uid . $u }{'board_notifications'} ) ) {
                $b{$_} = 1;
            }
...
sub getMailFiles {
    ...
    @bmaildir =
      map { ( split( /\./, $_ ) )[0] } grep { /\.mail$/ } readdir(BOARDNOT);
    closedir(BOARDNOT);
    opendir( THREADNOT, "$datadir" );
    @tmaildir =
      map { ( split( /\./, $_ ) )[0] } grep { /\.mail$/ } readdir(THREADNOT);
    closedir(THREADNOT);
}
...
<script language="Javascript1.2" type="text/javascript">
    ...
    foreach ( keys %$board_notify ) {    # boardname, boardnotifytype , new
        ...
        if ( ${ $$board_notify{$_} }[2] ) {
            $new =
qq~<img src="$imagesdir/on.gif" alt="$notify_txt{'333'}" title="$notify_txt{'333'}" border="0" style="vertical-align: middle;" />~;
        }
        else {
            $new =
qq~<img src="$imagesdir/off.gif" alt="$notify_txt{'334'}" title="$notify_txt{'334'}" border="0" style="vertical-align: middle;" />~;
        }
...
sub NotificationAlert {
    ...
    &BoardTotals( "load", @noloadboard )
      if @noloadboard;    # to get ${$uid.$myboard}{'lastposttime'}
...
qq~<a href="$scripturl?action=viewprofile;username=$useraccount{$musername}">${$uid.$musername}{'realname'}</a>~;
            ...
            if ($max_log_days_old) {
               ...
               # and compare to the last post time on the thread.
                ...
                {
                    $new =
qq~<img src="$imagesdir/new.gif" alt="$notify_txt{'335'}" title="$notify_txt{'335'}" border="0"/>~;
                }
            }
