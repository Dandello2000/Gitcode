...
$pollplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub DoVote {
    ...
    for ( my $i = 0 ; $i < @polled ; $i++ ) {
        ( $voters_ip, $voters_name, $voters_vote, $vote_time ) =
          split( /\|/, $polled[$i] );
        chomp $voters_vote;
        if (   $iamguest
...
sub UndoVote {
    ...
    &check_deletepoll;
    if ( !$iamadmin && $poll_nodelete{$username} ) {
...
sub LockPoll {
    ...
    fclose(FILE);
    chomp $poll_question;
    ( $poll_question, $poll_locked, $poll_uname, $poll_stuff ) =
      split( /\|/, $poll_question, 4 );
    unless ( $username eq $poll_uname || $iamadmin || $iamgmod || $iammod ) {
...
sub votedetails {
    ...
    if ( $poll_modname ne '' && $poll_mod ne '' ) {
        ...
        $displaydate =
qq~<span class="small">&#171; $polltxt{'45a'}: <a href="$scripturl?action=viewprofile;username=$useraccount{$poll_modname}">${$uid.$poll_modname}{'realname'}</a> $polltxt{'46'}: $poll_mod &#187;</span>~;
    }
    if ( $poll_uname ne '' && $poll_date ne '' ) {
        ...
        if ( $poll_uname ne 'Guest' && -e "$memberdir/$poll_uname.vars" ) {
            &LoadUser($poll_uname);
            $displaydate =
qq~<span class="small">&#171; $polltxt{'45'}: <a href="$scripturl?action=viewprofile;username=$useraccount{$poll_uname}">${$uid.$poll_uname}{'realname'}</a> $polltxt{'46'}: $poll_date &#187;</span>~;
        }
        else {
            $displaydate =
...
<table cellpadding="4" cellspacing="1" border="0" width="90%" class="bordercolor" align="center">
    ...
    foreach $entry (@polled) {
        ...
        if ( $voters_name ne 'Guest' && -e "$memberdir/$voters_name.vars" ) {
            ...
            $voters_name =
qq~<a href="$scripturl?action=viewprofile;username=$useraccount{$voters_name}">${$uid.$voters_name}{'realname'}</a>~;
        }
        foreach $oldvote ( split( /\,/, $voters_vote ) ) {
            if ($ubbcpolls) {
...
qq~<input type="radio" name="option" id="option$i" value="$i" style="margin: 0; padding: 0; vertical-align: middle;" />~;
    ...
    if ( !$poll_locked && $poll_end ) {
        ...
        $poll_end .=
          "$days $post_polltxt{'100a'}"
          . ( $hours ? ", " : " $post_polltxt{'100c'} " )
          if $days;
...
sub check_deletepoll {
    ...
    if ( -e "$datadir/$pollnum.polled" ) {
        ...
        foreach $chvoter (@chpolled) {
            ( undef, $chvotersname, undef, $chvotedate ) =
              split( /\|/, $chvoter );
