...
$postplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub Post {
    ...
    my (
        $subtitle, $x,        $mnum,      $msub,  $mname,  $memail,
        $mdate,    $mreplies, $musername, $micon, $mstate, $msubject,
    ...
    if ( $mstate =~ /l/i && $bypass_lock_perm ) {
    ...
    if ( $threadid ne '' ) {
        ...
        if ( $quotemsg ne '' ) {
            ...
            ) = split( /\|/, ${ $thread_arrayref{$threadid} }[$quotemsg] );
            $message = $mmessage;
            $message =~ s~<br.*?>~\n~ig;
            $message =~ s/ \&nbsp; \&nbsp; \&nbsp;/\t/ig;
            if ( !$nestedquotes ) {
...
s~\n{0,1}\[quote([^\]]*)\](.*?)\[/quote([^\]]*)\]\n{0,1}~\n~isg;
            ...
            $usernames_life_quote{$hidename} =
              $mname;    # for display names in Quotes in LivePreview
            my $maxlengthofquote =
              $MaxMessLen -
              length(
...
qq~[quote author=$hidename link=$threadid/$quotemsg#$quotemsg date=$mdate\]$message\[/quote\]\n~;
            ...
            (
                $msubject, $mname,   $memail, $mdate,    $musername,
                $micon,    $mattach, $mip,    $mmessage, $mns
            ) = split( /\|/, ${ $thread_arrayref{$threadid} }[0] );
        }
        $msubject =~ s/\bre:\s+//ig;
        $sub        = "Re: $msubject";
        $settofield = 'message';
    ...
    if ( $ENV{'HTTP_USER_AGENT'} =~ /(MSIE) (\d)/ ) {
        if   ( $2 >= 7.0 ) { $iecopycheck = ''; }
        else               { $iecopycheck = qq~ checked="checked"~; }
    }
...
qq~<form action="$scripturl?$thecurboard" method="post" name="postmodify" enctype="application/x-www-form-urlencoded" onsubmit="if(!checkForm(this)) {return false} else {return submitproc()}">~;
    ...
    if ( $postthread == 2 ) {
        $yymain .=
          qq~<input type="hidden" id="title" name="PostReply" value="title" />~;
    }
    ...
    if ( $showadded == 1 ) {
        while ( $SmilieURL[$i] ) {
            ...
            $moresmilieslist .=
qq~                   document.write('<img src="$tmpurl" align="bottom" alt="$SmilieDescription[$i]" title="$SmilieDescription[$i]" border="0" onclick="javascript: MoreSmilies($i);" style="cursor: pointer;" />$SmilieLinebreak[$i] ');\n~;
    ...
    if ( $showsmdir == 1 ) {
        ...
        foreach $line ( sort { uc($a) cmp uc($b) } @contents ) {
                ...
                || $extension =~ /png/i )
            {
                if ( $line !~ /banner/i ) {
                    $moresmilieslist .=
qq~                   document.write('<img src="$smiliesurl/$line" align="bottom" alt="$name" title="$name" border="0" onclick="javascript: MoreSmilies($i);" style="cursor: hand;" />$SmilieLinebreak[$i] ');\n~;
...
~;
    ...
    if ( keys(%moderatorgroups) > 0 ) {
        while ( $_ = each(%moderatorgroups) ) {
            ...
            foreach $memberaddgroups (
                split( /,/, ${ $uid . $username }{'addgroups'} ) )
            {
...
qq~<input type="hidden" value="$thestatus" name="topicstatus" />~;
      ...
      <tr>
          ...
          . ( $set_subjectMaxLength + ( $sub =~ /^Re: / ? 4 : 0 ) )
      ...
      <tr>
          ...
          . qq~" valign="middle" class="windowbg2" width="77%">
            ~;
...
<script type="text/javascript" src="$yyhtml_root/googiespell/cookiesupport.js"></script>~;
      ...
      <tr id="feature_status_5">
            ...
            $yymain .= qq~
                  ...
                  <img name="attform_sub" id="attform_sub" src="$defaultimagesdir/cat_collapse.gif" alt="$fatxt{'80s'}" title="$fatxt{'80s'}" border="0" style="cursor:pointer; visibility:hidden;" onclick="enabPrev2(-1);" />~
              if $allowattach > 1;
            ...
            for ( my $y = 1 ; $y <= $allowattach ; $y++ ) {
                    ...
                    && -e "$uploaddir/$files[$y-1]" )
                {
                  ...
                  <div id="attform_b_$y" style="float:left; width:76%;~
                    ...
                    $yymain .= qq~
                  <div id="attform_a_$y" style="float:left; width:23%;~
                      . ( $y > 1 ? qq~ visibility:hidden; height:0px~ : '' )
                      . qq~"><b>$fatxt{'6'} $y:</b></div>
                  <div id="attform_b_$y" style="float:left; width:76%;~
                      . ( $y > 1 ? qq~ visibility:hidden; height:0px~ : '' )
...
qq~<script type="text/javascript" src="$yyhtml_root/googiespell/cookiesupport.js"></script>~
      unless $yyinlinestyle =~ /cookiesupport\.js/;
    $yymain .= qq~
                  //-->
                  </script>
                  <br /><br />
...
~
          . ( !$Quick_Post ? "document.postmodify.$settofield.focus();" : "" )
          . qq~\n\n~;
...
qq~$FORM{'messageheight'}|$FORM{'messagewidth'}|$FORM{'txtsize'}|$FORM{'col_row'}~;
    ...
    $testmessage =~ s/\[.*?\]//g;
    if ( $testmessage eq "" && $message ne "" && $pollthread != 2 ) {
...
sub Post2 {
    ...
    my (
        $email,     $ns,           $notify,    $hasnotify, @memberlist,
        $i,         $membername,   $testname,  @reserve,   @reservecfg,
        $matchword, $matchcase,    $matchuser, $matchname, $namecheck,
        $reserved,  $reservecheck, $mnum,      $msub,      $mname,
        $memail,    $mdate,        $musername, $micon,     $mstate,
        $pageindex, $tempname
    );
    ...
    if ( !$threadid ) {
        ...
        unless ( &AccessCheck( $currentboard, 1 ) eq 'granted' || $pollthread )
        {
        ...
        unless ( &AccessCheck( $currentboard, 2 ) eq 'granted' || $pollthread )
        {
    ...
    if ($pollthread) {
        ...
        unless ( &AccessCheck( $currentboard, 3 ) eq 'granted' ) {
    ...
    for ( my $y = 1 ; $y <= $allowattach ; ++$y ) {
        if ( $CGI_query && $CGI_query->upload("file$y") ) {
            ...
            unless ( &AccessCheck( $currentboard, 4 ) eq 'granted' ) {
    ...
    &Preview( $post_txt{'76'} )
      unless ( ${ $uid . $username }{'email'} || $email );
    &Preview( $post_txt{'77'} )
      unless ( $subject && $subject !~ m~\A[\s_.,]+\Z~ );
    ...
    if ( length($mess_len) > $MaxMessLen ) {
        &Preview( $post_txt{'536'} . " "
              . ( length($mess_len) - $MaxMessLen ) . " "
              . $post_txt{'537'} );
    }
    undef $mess_len;
    ...
    for ( my $y = 1 ; $y <= $allowattach ; ++$y ) {
        ...
        if ($file) {
            ...
            if ($match) {
                unless (
                    $allowattach
                    && ( ( $allowguestattach == 0 && $username ne 'Guest' )
                        || $allowguestattach == 1 )
                  )
                {
                    foreach (@filelist) { unlink("$uploaddir/$_"); }
...
qq~$newthreadid|$mreplies|$subject|$name|$currentboard|$filesizekb{$fixfile}|$date|$fixfile|0\n~;
        ...
        if ( $mstate =~ /l/i ) {    # locked thread
            my $icanbypass = &checkUserLockBypass
              if $bypass_lock_perm;    # only if bypass switched on
            ...
            if ( ${ $uid . $username }{'position'} ) {
                ...
                foreach $postamount ( sort { $b <=> $a } keys %Post ) {
                    if ( ${ $uid . $username }{'postcount'} >= $postamount ) {
                        ( $title, undef ) =
                          split( /\|/, $Post{$postamount}, 2 );
                        $grp_after = $title;
                        last;
                    }
                }
            }
            &ManageMemberinfo( "update", $username, '', '', $grp_after,
                ${ $uid . $username }{'postcount'} );
        }
...
qq~$post_cutts{'3'} $post_cutts{'3a'} <a href="$scripturl?action=post;num=$threadid;title=PostReply/$INFO{'start'}" style="text-decoration: underline;"> $post_cutts{'4'}</a> $post_cutts{'6'} ~;
        ...
        for ( $amounter = 0 ; $amounter < $cutamount ; $amounter++ ) {
            ...
            ) = split( /\|/, $messages[$amounter] );
            $messagedate = $tempdate;
            $tempdate    = &timeformat($tempdate);
            $parseflash  = 0;

            if ( $tempname ne 'Guest' && -e ("$memberdir/$tempname.vars") ) {
                &LoadUser($tempname);
            }
            ...
            {
                $displaynamelink =
qq~<a href="$scripturl?action=viewprofile;username=$useraccount{$tempname}" class="catbg a">${$uid.$tempname}{'realname'}</a>~;
            }
            ...
            $quote_mname =~ s/'/\\'/g;
            my $quote_msg_id =
              $tsreverse == 1 ? ( @messages - $amounter - 1 ) : $amounter;
...
sub modAlert2 {
    ...
    if ($iamguest) {
        ...
        $name =~ s/\s+\Z//;
        &Preview( $post_txt{'75'} )
          unless ( $name ne '' && $name ne '_' && $name ne ' ' );
...
# Send notification (Will only work if Admin has allowed the Email Notification)
                ...
                && ${ $uid . $toBoardMod }{'email'} ne '' )
            {
                ...
                &sendmail( ${ $uid . $toBoardMod }{'email'},
                    $notify_txt{'145'}, $chmessage, '', $emailcharset );

            }
            elsif ( $PMenableBm_level && $x ) {
                &ManageMemberinfo("load") if !%memberinf;
                map {
                    if ( $_ && $modgrps =~ /\b$_\b/ ) { next managemods; }
                } split( /,/, ( split( /\|/, $memberinf{$toBoardMod} ) )[4] );
            }
