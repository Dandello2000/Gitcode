...
$profileplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub ProfileCheck2 {
    ...
    $yySetLocation =
      ...
      . ( $INFO{'newpassword'} ? ";newpassword=1" : "" );
    &redirectexit;
}
...
qq~&rsaquo; <a href="$scripturl?action=mycenter" class="nav">$img_txt{'mycenter'}</a> &rsaquo; $profiletitle~;
    ...
    &CalcAge( $user, "parse" );
    $dayormonthm =
qq~<label for="bday1">$profile_txt{'564'}</label><input type="text" name="bday1" id="bday1" size="2" maxlength="2" value="$umonth" /> ~;
    $dayormonthd =
qq~<label for="bday2">$profile_txt{'565'}</label><input type="text" name="bday2" id="bday2" size="2" maxlength="2" value="$uday" /> ~;
...
<table cellspacing="1" cellpadding="4" width="100%" align="center" class="bordercolor" border="0">
      <tr>
            <td class="catbg" colspan="2"><img src="$imagesdir/profile.gif" alt="" border="0" /> <b>$profiletitle</b><br /><span class="small">$profile_txt{'698'}</span>~
      . ( $INFO{'newpassword'} ? $profile_txt{'80'} : "" ) . qq~</td>
      </tr>
      <tr class="windowbg">
...
<form action="$scripturl?action=$scriptAction;username=$useraccount{$INFO{'username'}};sid=$INFO{'sid'}" method="post" name="creator"~
      . (
        ...
        ? qq~ enctype="multipart/form-data"~
        : ""
      )
      . qq~>
<table cellspacing="1" cellpadding="4" width="100%" align="center" class="bordercolor" border="0">
...
qq~                <option value="$line"$checked>$name</option>\n~;
        ...
        $showProfile .= qq~
            <td align="left"><label for="userpic"><b>$profile_txt{'229'}:</b></label><br /><span class="small"><label for="userpic">$profile_txt{'474'}</label><label for="userpicpersonalcheck">$profile_txt{'475'}</label>~
          . (
            $upload_useravatar
            ? qq~<br />
    ...
    $showProfile .= qq~
      ...
      <tr class="windowbg">
            ...
            <td align="left"><span class="small">$profile_txt{'373'}:<br /><b>~
      . &timeformat( $date, 1 )
      . qq~</b><br /><br /></span><select name="usertimesign" id="usertimesign"><option value="">+</option><option value="-"~
      . ( $usertimeoffset[0] < 0 ? ' selected="selected"' : '' )
      . qq~>-</option></select>
    ...
    for ( my $i = 0 ; 60 > $i ; $i++ ) {
        ...
        $showProfile .=
            qq~\n               <option value="$j"~
          . ( $usertimeoffset[1] eq $j ? ' selected="selected"' : '' ) . qq~>~
          . sprintf( "%02d", $i )
          . qq~</option>~;
    }
    $showProfile .= qq~
      ...
      <tr class="windowbg">
            ...
            <td align="left"><input type="checkbox" name="dsttimeoffset" id="dsttimeoffset" value="1"~
      . ( ${ $uid . $user }{'dsttimeoffset'} ? ' checked="checked"' : '' )
      . qq~ /></td>
      </tr>
      <tr class="windowbg">
            <td align="left"><label for="dynamic_clock"><b>$profile_txt{'520'}</b><br /><span class="small">$profile_txt{'521'}</span></label></td>
            <td align="left"><input type="checkbox" name="dynamic_clock" id="dynamic_clock" value="1"~
      . ( ${ $uid . $user }{'dynamic_clock'} ? ' checked="checked"' : '' )
      . qq~ /></td>
      </tr>~;
...
# I think it can be deleted around version 2.4.0 without causing mayor issues (deti).
        ...
        || $enable_notifications == 3 )
    {
      ...
      <tr class="windowbg">
        ...
        $showProfile .= qq~
          ...
          . ( ${ $uid . $user }{'onlinealert'} ? ' checked="checked"' : '' )
    ...
    if ($ttsureverse) {
        ${ $uid . $user }{'reversetopic'} = $ttsreverse
          unless exists( ${ $uid . $user }{'reversetopic'} );
        $showProfile .= qq~
      <tr class="windowbg">
            <td align="left"><label for="reversetopic"><b>$profile_txt{'810'}</b><br /><span class="small">$profile_txt{'811'}</span></label></td>
            <td align="left" valign="top"><input type="checkbox" name="reversetopic" id="reversetopic"~
          . ( ${ $uid . $user }{'reversetopic'} ? qq~ checked="checked"~ : '' )
          . qq~ /></td>
      </tr>~;
    }
    ...
    if ( $enable_notifications > 1 ) {
      ...
      <tr class="windowbg">
            ...
            <td align="left">
                  ...
                  <option value="0"~
          . ( ${ $uid . $user }{'notify_me'} < 2 ? '' : ' selected="selected"' )
          . qq~>$profile_txt{'164'}</option>
                  <option value="1"~
          . ( ${ $uid . $user }{'notify_me'} > 1 ? ' selected="selected"' : '' )
          . qq~>$profile_txt{'163'}</option>
                  </select>
            </td>
      </tr>~;
...
sub ModifyProfileAdmin {
    ...
    if (%NoPost) {
        ( $tta, $selsize ) =
          &DrawGroups( ${ $uid . $user }{'addgroups'}, '', 1 );
    }
    ...
    if ( $member{'moda'} eq $profile_txt{'88'} ) {
        ...
        if ( ${ $uid . $user }{'realname'} ne $member{'name'} ) {
            ...
            if ( &Censor( $member{'name'} ) ne $member{'name'} ) {
                &fatal_error( "name_censored",
                    &CheckCensor("$member{'name'}") );
            }
            ...
            for ( my $i = 0 ; $i < @attachments ; $i++ ) {
                $attachments[$i] =~
s/^(\d+\|\d+\|.*?)\|(.*?)\|/ ($2 eq ${$uid.$user}{'realname'} ? "$1|$member{'name'}|" : "$1|$2|") /e;
            }
...
qq~$scripturl?action=$scriptAction;username=$useraccount{$member{'username'}};sid=$INFO{'sid'}~;
        ...
        unlink("$facesdir/UserAvatars/$1")
          if ${ $uid . $user }{'userpic'}
              && ${ $uid . $user }{'userpic'} =~ /$facesurl\/UserAvatars\/(.+)/;
...
sub ModifyProfileContacts2 {
    ...
    &fatal_error("invalid_email")
      if ( $member{'email'} =~ /(@.*@)|(\.\.)|(@\.)|(\.@)|(^\.)|(\.$)/ )
      || (
        $member{'email'} !~ /^.+@\[?(\w|[-.])+\.[a-zA-Z]{2,4}|[0-9]{1,4}\]?$/ );
    &LoadCensorList;
...
# Since we haven't encountered a fatal error, time to rewrite our memberlist a little.
    ...
    if ( $enable_MCaway && $member{'offlinestatus'} eq '' ) {
    ...
    ${ $uid . $user }{'weburl'}   = (
        ( $member{'weburl'} && $member{'weburl'} !~ m~\Ahttps?://~ )
        ? "http://"
        : ""
    ) . $member{'weburl'};
...
sub ModifyProfileOptions2 {
    ...
    if (   ${ $uid . $user }{'postcount'} < $minlinksig
        ...
        && !$iamgmod )
    {
        if (   $member{'signature'} =~ m~http:\/\/~
            || $member{'signature'} =~ m~https:\/\/~
            || $member{'signature'} =~ m~ftp:\/\/~
            || $member{'signature'} =~ m~www.~
            || $member{'signature'} =~ m~ftp.~ =~ m~\[url~
            || $member{'signature'} =~ m~\[link~
            || $member{'signature'} =~ m~\[img~
            || $member{'signature'} =~ m~\[ftp~ )
        {
    ...
    if ( $allowpics && $upload_useravatar && $file ) {
        ...
        unlink("$facesdir/UserAvatars/$1")
          if ${ $uid . $user }{'userpic'} =~ /$facesurl\/UserAvatars\/(.+)/;
 ...
 # create a new file on the server using the formatted ( new instance ) filename
    ...
    if (   $member{'usertimesign'} !~ /^-?$/
        || $member{'usertimehour'} !~ /^\d+$/
        || $member{'usertimemin'} !~ /^\d+$/ )
    {
...
"$member{'usertimesign'}$member{'usertimehour'}.$member{'usertimemin'}"
    ...
    if ( $addmemgroup_enabled > 1 ) {
        ...
        foreach ( keys %NoPost ) {
            ...
            if ( $groups{$_} == 1 && ( split /\|/, $NoPost{$_} )[10] ) {
                push( @nopostmember, $_ );
            }
            elsif ( $groups{$_} == 2 && !( split /\|/, $NoPost{$_} )[10] ) {
                push( @nopostmember, $_ );
            }
        }
        $member{'joinmemgroup'} = join( ',', @nopostmember );
        if ( $member{'joinmemgroup'} eq '' ) {
            $member{'joinmemgroup'} = "###blank###";
        }
        if ( $member{'joinmemgroup'} ne ${ $uid . $user }{'addgroups'} ) {
            &ManageMemberinfo( "update", $user, '', '', '', '',
                $member{'joinmemgroup'} );
        }
...
qq~$dr_month/$dr_day/$dr_year $maintxt{'107'} $dr_hour:$dr_minute:$dr_secund~;
        ...
        || $member{'settings7'} ne ${ $uid . $user }{'position'} )
    {
        ...
        &ManageMemberinfo( "update", $user, '', '', $grp_after,
            $member{'settings6'} );
    }
...
sub ViewProfile {
    ...
    my (
        ...
        $row_myspace,   $row_facebook, $row_email,  $row_website,
        $row_signature, $showusertext
    );
    ...
    if ($allowpics) {
        if ( ${ $uid . $user }{'userpic'} eq "blank.gif" ) {
            $pic =
qq~<img src="$imagesdir/nn.gif" name="avatar_img_resize" alt="" border="0" style="display:none" />~;
        }
        elsif ( ${ $uid . $user }{'userpic'} =~ /^https?:\/\// ) {
            $pic =
qq~<img src="${$uid.$user}{'userpic'}" name="avatar_img_resize" alt="" border="0" style="display:none" />~;
        }
        else {
            $pic =
qq~<img src="$facesurl/${$uid.$user}{'userpic'}" name="avatar_img_resize" alt="" border="0" style="display:none" />~;
        }
        $pic_row =
qq~<div style="float: left; width: 20%; text-align: center; padding: 5px 5px 5px 0px;">
    ...
    if ( $buddyListEnabled && $user ne $username ) {
        ...
        $buddybutton = "<br />"
          . (
            $mybuddie{$user}
            ? qq~<img src="$imagesdir/buddylist.gif" border="0" align="middle" alt="$display_txt{'isbuddy'}" /> $display_txt{'isbuddy'}~
    ...
    if ( ${ $uid . $user }{'facebook'} ) {
        $row_facebook = qq~
          ...
          . ( ${ $uid . $user }{'facebook'} !~ /\D/ ? "profile.php?id=" : "" )
...
<table border="0" cellpadding="8" cellspacing="1" class="bordercolor" align="center" width="100%">~;
    ...
    if (   ( $iamadmin || $iamgmod && $gmod_access2{'ipban3'} )
        && !$view
        && $user ne $username
        && ${ $uid . $user }{'position'} ne 'Administrator' )
    {
        ...
        if ( $is_banned =~ /E/ ) {
            $ban_email_link =
qq~<a href="$adminurl?action=ipban3;ban_email=$ban_user_email;username=$useraccount{$user};unban=1"><span class="small">[$profile_txt{'904'}]</span></a>~;
        }
        else {
            $ban_email_link =
qq~<a href="$adminurl?action=ipban3;ban_email=$ban_user_email;username=$useraccount{$user}"><span class="small">[$profile_txt{'907'}]</span></a>~;
        }
...
qq~<a href="$adminurl?action=ipban3;ban_memname=$ban_user_name;username=$useraccount{$user};unban=1"><span class="small">[$profile_txt{'903'}]</span></a>~;
            ...
            $ban_user_link =
qq~<a href="$adminurl?action=ipban3;ban_memname=$ban_user_name;username=$useraccount{$user}"><span class="small">[$profile_txt{'906'}]</span></a>~;
        }

        # Shows the banning stuff for IP's
        if ( ${ $uid . $user }{'lastips'} ) {
            ( $ip_one, $ip_two, $ip_three ) =
              split( /\|/, ${ $uid . $user }{'lastips'} );

            if ( &check_banlist( "", "$ip_one", "" ) ) {
                $banlink_one =
qq~<a href="$adminurl?action=ipban3;ban=$ip_one;username=$useraccount{$user};unban=1"><span class="small">[$profile_txt{'905'}]</span></a>~;
            }
            else {
                $banlink_one =
qq~<a href="$adminurl?action=ipban3;ban=$ip_one;username=$useraccount{$user}"><span class="small">[$profile_txt{'908'}]</span></a>~;
            }

            if ( &check_banlist( "", "$ip_two", "" ) ) {
                $banlink_two =
qq~<a href="$adminurl?action=ipban3;ban=$ip_two;username=$useraccount{$user};unban=1"><span class="small">[$profile_txt{'905'}]</span></a>~;
            }
            else {
                $banlink_two =
qq~<a href="$adminurl?action=ipban3;ban=$ip_two;username=$useraccount{$user}"><span class="small">[$profile_txt{'908'}]</span></a>~;
            }

            if ( &check_banlist( "", "$ip_three", "" ) ) {
                $banlink_three =
qq~<a href="$adminurl?action=ipban3;ban=$ip_three;username=$useraccount{$user};unban=1"><span class="small">[$profile_txt{'905'}]</span></a>~;
            }
            else {
                $banlink_three =
qq~<a href="$adminurl?action=ipban3;ban=$ip_three;username=$useraccount{$user}"><span class="small">[$profile_txt{'908'}]</span></a>~;
            }

            if ($ip_one) {
                $ip_ban_options = qq~$ip_one<br />$banlink_one <br />~;
            }
            if ($ip_two) {
                $ip_ban_options .= qq~$ip_two<br />$banlink_two <br />~;
            }
            if ($ip_three) {
                $ip_ban_options .= qq~$ip_three<br />$banlink_three <br />~;
            }
        }
...
sub usersrecentposts {
    ...
    my (
        ...
        $curcat,            %boardcat,     %catinfos,    %catboards,
        $openmemgr,         @membergroups, $tmpa,        %openmemgr,
        $curboard,          @threads,      @boardinfo,   $i,
        $c,                 @messages,     $tnum,        $tsub,
        $tname,             $temail,       $tdate,       $treplies,
        $tusername,         $ticon,        $tstate,      $mname,
        $memail,            $mdate,        $musername,   $micon,
        $mattach,           $mip,          $mns,         $counter,
        $board,             $notify,       $catid
    );

    &Recent_Load($curuser);
    my @recent =
      sort { ${ $recent{$b} }[1] <=> ${ $recent{$a} }[1] }
  ...
  recentcheck: foreach $thread (@recent) {
        ...
        if ( !$boardtxt{$curboard} ) {
            ...
            if (
                ...
                && (  !&CatAccess( ${ $catinfos{$curboard} }[1] )
                    || &AccessCheck( $curboard, '', $boardperms ) ne "granted" )
              )
            {
                $recentcount--;
                next recentcheck;
    ...
    if ( $recentfound < $display && $numfound < $recentcount ) {
      categorycheck: foreach $catid (@categoryorder) {
            if ( !&CatAccess( ( split( /\|/, $catinfo{$catid}, 3 ) )[1] ) ) {
                next categorycheck;
            }
          ...
          boardcheck: foreach $curboard ( split( /\,/, $cat{$catid} ) ) {
                if ( !$boardtxt{$curboard} ) {
                    ...
                    if (  !$iamadmin
                        && &AccessCheck( $curboard, '', $boardperms ) ne
                        "granted" )
                    {
                        next boardcheck;
                    }
...
qq~<a href="$scripturl?board=$board;action=post;num=$tnum/$c#$c;title=PostReply">$img{'reply'}</a>$menusep<a href="$scripturl?board=$board;action=post;num=$tnum;quote=$c;title=PostReply">$img{'recentquote'}</a>$notify &nbsp;~;
    ...
    if ( !$counter ) {
        ...
        $showProfile .=
qq~<p align=left><a href="$scripturl?action=viewprofile;username=$useraccount{$curuser}"><b>$profile_txt{'92'} ${$uid.$curuser}{'realname'}</b></a></p>~;
    }
