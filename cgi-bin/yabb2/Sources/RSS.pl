...
$rssplver = 'YaBB 2.6 $Revision: 1.0 $';
    ...
    foreach (@threadlist) {
        ...
        if ($accept_permalink) {
            ...
            $yymain .= qq~          <item>
              ...
              . qq~</link>
                        <category>~
              . &RSSDescriptionTrim($category) . qq~</category>
...
~;
            ...
            $yymain .= qq~          <item>
                        <title>~ . &RSSDescriptionTrim($msub) . qq~</title>
                        <link>~
              . &RSSDescriptionTrim("$scripturl?num=$curnum") . qq~</link>
                        <category>~
              . &RSSDescriptionTrim($category) . qq~</category>
                        <guid>~
              . &RSSDescriptionTrim("$scripturl?num=$curnum") . qq~</guid>
~;
        }
...
"${$uid.$musername}{'email'} (${$uid.$musername}{'realname'})"
        ...
        if ($showdate) {
            $mdate = $curnum
              if $rss_message == 2;    # Sort by topic creation if requested.
...
<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">
      ...
      . qq~" rel="self" type="application/rss+xml" />
            ...
            <language>~
      . &RSSDescriptionTrim("$maintxt{'w3c_lngcode'}") . qq~</language>
