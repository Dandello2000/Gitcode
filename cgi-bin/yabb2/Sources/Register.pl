...
$registerplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub Register {
    ...
    my (
        ...
        $tmpregpasswrd2, $hidechecked, @birthdate
    );
    ...
    if ($adomains) {
        ...
        $aedomains =
qq~<table border="0" width="100%" cellspacing="0" cellpadding="0"><tr><td><input type="text" maxlength="100" name="email" id="email" value="$tmpregemail" size="15" /></td><td><select name="domain" id="domain">~;
        foreach (@domains) {
            $aedomains .=
              ( $_ =~ m/\@/ )
...
<!--user name section-->
    ...
    if ($birthday_on_reg) {
      ...
      <tr>
            ...
            <td class="windowbg2" align="left" valign="top"><input type="text" name="birth_month" id="birth_month" size="2" value="$birthdate[1]" maxlength="2" onkeypress="jumpatnext('document.creator.birth_month','document.creator.birth_day',2)" /> <input type="text" name="birth_day" id="birth_day" size="2" value="$birthdate[0]" maxlength="2" onkeypress="jumpatnext('document.creator.birth_day','document.creator.birth_year',2)" /> <input type="text" name="birth_year" id="birth_year" size="4" value="$birthdate[2]" maxlength="4" />~
              . ( $birthday_on_reg == 2 ? ' *' : '' )
      ...
      <tr>
            ...
            <td class="windowbg2" align="left" valign="top"><input type="text" name="birth_day" id="birth_day" size="2" value="$birthdate[0]" maxlength="2" onkeypress="jumpatnext('document.creator.birth_day','document.creator.birth_month',2)" /> <input type="text" name="birth_month" id="birth_month" size="2" value="$birthdate[1]" maxlength="2" onkeypress="jumpatnext('document.creator.birth_month','document.creator.birth_year',2)" /> <input type="text" name="birth_year" id="birth_year" size="4" value="$birthdate[2]" maxlength="4" />~
              . ( $birthday_on_reg == 2 ? ' *' : '' )
    ...
    if ($gender_on_reg) {
        if ( $gender_on_reg == 1 ) {
            $gender_req =
              qq~<label for="gender"><b>$register_txt{'gender'}: </b></label>~;
        }
        else {
            $gender_req =
qq~* <label for="gender"><b>$register_txt{'gender'}: </b></label>~;
    ...
    if ($extendedprofiles) {
        ...
        $reg_ext_prof =~
          s/align="left" valign="top"/align="right" valign="top"/g;
        $reg_ext_prof =~
          s/<\/td><td align="left">/<\/td><td class="windowbg2" align="left">/g;
        $yymain .= $reg_ext_prof;
    }
...
sub Register2 {
    ...
    if ( $RegAgree && $FORM{'regagree'} ne 'yes' ) {
...
# check if there is a system hash named like this by checking existence through size
    ...
    if ( &CheckCensor( $member{'regusername'} ) ne "" ) {
        &fatal_error( "censor1", &CheckCensor( $member{'regusername'} ) );
    }
    if ( &CheckCensor( $member{'email'} ) ne "" ) {
        &fatal_error( "censor2", &CheckCensor( $member{'email'} ) );
    }
    if ( &CheckCensor( $member{'regrealname'} ) ne "" ) {
    ...
    if ($en_spam_questions) {
        ...
        unless ($spam_questions_case) {
            ...
            $member{'verification_question'} =
              lc( $member{'verification_question'} );
        }
        &fatal_error("no_verification_question")
          if $member{'verification_question'} eq '';
        @verificationanswer = split( /,/, $verification_answer );
        foreach (@verificationanswer) {
            $_ =~ s/\A\s+//;
            $_ =~ s/\s+\Z//;
        }
        unless ( grep { $member{'verification_question'} eq $_ }
            @verificationanswer )
        {
    ...
    if (   $new_template !~ m^\A[0-9a-zA-Z\_\(\)\ \#\%\-\:\+\?\$\&\~\.\,\@]+\Z^
        && $new_template ne '' )
    {
...
"$member{'birth_month'}/$member{'birth_day'}/$member{'birth_year'}";
            ...
            &fatal_error( "", $register_txt{'error_birth_month'} )
              if length( $member{'birth_month'} ) < 2
                  || $member{'birth_month'} < 1
                  || $member{'birth_month'} > 12;
            &fatal_error( "", $register_txt{'error_birth_day'} )
              if length( $member{'birth_day'} ) < 2
                  || $member{'birth_day'} < 1
                  || $member{'birth_day'} > 31;
            &fatal_error( "", $register_txt{'error_birth_year'} )
              if length( $member{'birth_year'} ) < 4;
            &fatal_error( "", $register_txt{'error_birth_year_real'} )
...
"$member{'birth_month'}/$member{'birth_day'}/$member{'birth_year'}";
    ...
    if ( ( $addmemgroup_enabled == 1 || $addmemgroup_enabled == 3 )
        && $member{'joinmemgroup'} ne '' )
    {
        my @newmemgr;
        foreach ( split( /, /, $member{'joinmemgroup'} ) ) {
            if ( $NoPost{$_} && ( split /\|/, $NoPost{$_} )[10] == 1 ) {
                push( @newmemgr, $_ );
            }
        }
    ...
    foreach (@reglist) {
        ...
        # user is in list and the keys match, so move him/her for admin approval
                ...
                if ($emailpassword) {
                    ...
                    $yymain .=
qq~<br /><table border="0" width="100%" cellspacing="1" class="bordercolor" align="center">~;
...
"$messageid|$sendname|$reguser|||$imsubject|$date|$imtext|$messageid|0|$ENV{'REMOTE_ADDR'}|s|u||\n";
    ...
    if ( $regtype == 1 ) {
        ...
        if ( !$iamadmin && !$iamgmod ) {
            unless ($emailpassword) {
                $yymain .=
qq~<div class="bordercolor" style="width: 650px; margin-bottom: 8px; margin-left: auto; margin-right: auto;">~;
