...
$securityplver = 'YaBB 2.6 $Revision: 1.0 $';
...
# BIG board check
if ( $INFO{'board'} =~ m~/~ ) {
    ( $INFO{'board'}, $INFO{'start'} ) = split( '/', $INFO{'board'} );
}
if ( $INFO{'num'} =~ m~/~ ) {
    ( $INFO{'num'}, $INFO{'start'} ) = split( '/', $INFO{'num'} );
}
if ( $INFO{'letter'} =~ m~/~ ) {
    ( $INFO{'letter'}, $INFO{'start'} ) = split( '/', $INFO{'letter'} );
}
if ( $INFO{'thread'} =~ m~/~ ) {
    ( $INFO{'thread'}, $INFO{'start'} ) = split( '/', $INFO{'thread'} );
}
...
if ( $currentboard ne '' ) {
    if ( $currentboard !~ /\A[\s0-9A-Za-z#%+,-\.:=?@^_]+\Z/ ) {
...
sub CheckIcon {
    ...
    unless ( $icon eq "xx"
        ...
        || $icon eq "exclamation" )
    {
        unless ( $icon eq "question"
            || $icon eq "lamp"
            || $icon eq "smiley"
            || $icon eq "angry" )
        {
            unless ( $icon eq "cheesy"
                || $icon eq "grin"
                || $icon eq "sad"
                || $icon eq "wink" )
            {
...
sub AccessCheck {
    ...
    if ( $checktype == 1 ) {    # Post access check
        ...
        if ( $iamgmod || $boardmod ) { $access = "granted"; }
            ...
            @allowed_groups =
              split( /, /, ${ $uid . $curboard }{'replyperms'} );
            if ( ${ $uid . $curboard }{'replyperms'} eq "" ) {
                $access = "granted";
            }
