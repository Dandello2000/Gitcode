...
$spamcheckplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub spamcheck {
    ...
    $rawcontent =~ s/\[(.*?){1,2}\]//g
      ; # rip out all make up yabb tags if it is a non yabbc message which can be uses to break and obscure words
    $rawcontent =~ s/\<(.*?){1,2}\>//g
      ; # rip out all make up html tags if it is a html message which can be uses to break and obscure words
