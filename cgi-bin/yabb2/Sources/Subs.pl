...
$subsplver = 'YaBB 2.6 $Revision: 1.0 $';
...
sub ImgLoc {
    return (
        !-e "$forumstylesdir/$useimages/$_[0]"
...
qq~<img src="$imagesdir/tabsep211.png" border="0" alt="" style="vertical-align: middle;" />~
      if !$yynavback;
    $yynavback .=
qq~ <a href="#pagetop" class="nav">$img_txt{'102'}</a> <img src="$imagesdir/tabsep211.png" border="0" alt="" style="vertical-align: middle;" />~;
...
qq~$menusep<a href="$boardurl/AdminIndex.$yyaext">$img{'admin'}</a>~;
        ...
        if ( $sessionvalid == 0 && !$iamguest ) {
            my $sesredir;
            unless ( !$testenv
                || $action eq "revalidatesession"
                || $action eq "revalidatesession2" )
            {
                $sesredir = $testenv;
                $sesredir =~ s/\=/\~/g;
                $sesredir =~ s/;/x3B/g;
...
# We don't want to require since it's an error and trying to do anything extra for an error could be bad
    if ( $output =~ m~<yabb copyright>~ || $output =~ m~{yabb copyright}~ ) {
        $yycopyin = 1;
        ...
        if ($shownewsfader) {
            ...
            for ( my $j = 0 ; $j < @newsmessages ; $j++ ) {
                ...
                if ($enable_ubbc) {
                    ...
                    &DoUBBC;
                    $message =~
                      s/ style="display:none"/ style="display:block"/g;
                }
                &wrap2;
                $message =~ s/"/\\"/g;
                &ToChars($message);
    ...
    $output =~
      s~(a href=\S+?action=viewprofile;username=.+?)>~$1 rel="nofollow">~isg;
    if ( $imagesdir ne $defaultimagesdir ) {
        $output =~
s~img src=(\\*"|')$imagesdir/(.+?)(\1)~ "img src=$1" . &ImgLoc($2) . $3 ~eisg;
        $output =~
          s~\.src='$imagesdir/(.+?)'~ ".src='" . &ImgLoc($1) . "'" ~eisg
          ;    # For Javascript generated images
        $output =~
s~input type="image" src="$imagesdir/(.+?)"~ 'input type="image" src="' . &ImgLoc($1) . '"' ~eisg
          ;    # For input images
        $output =~
s~option value="$imagesdir/(.+?)"~ 'option value="' . &ImgLoc($1) . '"' ~eisg
          ;    # For the post page
    }
    $output =~
s~</form>~<input type="hidden" name="formsession" value="$formsession" /></form>~g;
    ...
    if ( $yycopyin == 0 ) {
        $output =
q~<center><h1><b>Sorry, the copyright tag <yabb copyright> must be in the template.<br />Please notify this forum's administrator that this site is using an ILLEGAL copy of YaBB!</b></h1></center>~;
    }
...
sub image_resize {
    ...
    $output =~
s/"((avatar|post|attach|signat)_img_resize)"([^>]*>)/ &check_image_resize($1,$2,$3) /ge;

    sub check_image_resize {
        my @x = @_;
        if ( $fix_avatar_img_size && $perl_do_it == 1 && $x[1] eq 'avatar' ) {
            if ( $max_avatar_width && $x[2] !~ / width=./ ) {
                $x[2] =~ s/( style=.)/$1width:$max_avatar_width\px;/;
            }
            if ( $max_avatar_height && $x[2] !~ / height=./ ) {
                $x[2] =~ s/( style=.)/$1height:$max_avatar_height\px;/;
            }
            $x[2] =~ s/display:none/display:inline/;
        }
        elsif ( $fix_post_img_size && $perl_do_it == 1 && $x[1] eq 'post' ) {
            if ( $max_post_width && $x[2] !~ / width=./ ) {
                $x[2] =~ s/( style=.)/$1width:$max_post_width\px;/;
            }
            if ( $max_post_height && $x[2] !~ / height=./ ) {
                $x[2] =~ s/( style=.)/$1height:$max_post_height\px;/;
            }
            $x[2] =~ s/display:none/display:inline/;
        }
        elsif ( $fix_attach_img_size && $perl_do_it == 1 && $x[1] eq 'attach' )
        {
            if ( $max_attach_width && $x[2] !~ / width=./ ) {
                $x[2] =~ s/( style=.)/$1width:$max_attach_width\px;/;
            }
            if ( $max_attach_height && $x[2] !~ / height=./ ) {
                $x[2] =~ s/( style=.)/$1height:$max_attach_height\px;/;
            }
            $x[2] =~ s/display:none/display:inline/;
        }
        elsif ( $fix_signat_img_size && $perl_do_it == 1 && $x[1] eq 'signat' )
        {
            if ( $max_signat_width && $x[2] !~ / width=./ ) {
                $x[2] =~ s/( style=.)/$1width:$max_signat_width\px;/;
            }
            if ( $max_signat_height && $x[2] !~ / height=./ ) {
                $x[2] =~ s/( style=.)/$1height:$max_signat_height\px;/;
            }
            $x[2] =~ s/display:none/display:inline/;
        }
        else {
...
sub readform {
    ...
    if ( substr( $ENV{QUERY_STRING}, 0, 1 ) eq "/" && $accept_permalink ) {
    ...
    if ( $ENV{QUERY_STRING} =~ m/action\=dereferer/ ) {
        ...
        $testenv =~ s/\&/\;/g;
        if ( $testenv && $debug ) {
            $getpairs = qq~<br /><u>$debug_txt{'getpairs'}:</u><br />~;
        }
    }
...
# URL encoding for web.de http://www.blooberry.com/indexdot/html/topics/urlencoding.htm
    ...
    if ( $ENV{REQUEST_METHOD} eq 'POST' ) {
        if ($debug) {
            $getpairs .= qq~<br /><u>$debug_txt{'postpairs'}:</u><br />~;
        }
        ...
        # uploaded data is larger. Change this values if you get error messages.
            ...
            foreach $name ( $CGI_query->param() ) {
                next
                  if $name =~ /^file(\d+|_avatar)$/
                ; # files are directly called in Profile.pl, Post.pl and ModifyMessages.pl
...
qq~[$debug_txt{'name'}-&gt;]$name=@value\[&lt;-$debug_txt{'value'}]<br />~;
                ...
                $FORM{$name} =
                  join( ', ', @value );    # multiple values are joined
            }
        }
        else {
    ...
    foreach $catid (@categoryorder) {
        ...
        foreach $board (@bdlist) {
            ...
            if ( $board eq $currentboard ) {
                $selecthtml .=
                  $INFO{'num'}
                  ? "    <option value=\"board=$board\" class=\"forumcurrentboard\">&nbsp; - $boardname &#171;&#171;</option>\n"
                  : " <option selected=\"selected\" value=\"board=$board\" class=\"forumcurrentboard\">&raquo;&raquo; $boardname</option>\n";
            }
            else {
                $selecthtml .=
" <option value=\"board=$board\">&nbsp; - $boardname</option>\n";
            }
        }
    }
...
sub spam_protection {
    ...
    if ( $flood && !$iamadmin && $action eq 'post2' ) {
...
sub enc_eMail {
    ...
    for ( my $i = 0 ; $i < $email_length ; $i++ ) {
        $code2 .= chr(
            ( ord( substr( $code1, $i, 1 ) ) ^ ord( substr( $email, $i, 1 ) ) )
        );
    }
...
sub Split_Splice_Move {
    ...
    elsif ( $s_s_m =~ /\[m by=(.+?) destboard=(.+?) dest=(.+?)\]/ )
...
qq~<b>$maintxt{'160c'}</b> <a href="$scripturl?num=$dest"><i><b>$maintxt{'160d'}</b></i></a><b> $maintxt{'525'} <i>${$uid.$mover}{'realname'}</i></b>~,
    ...
    $ssm += $s_s_m =~ s/\[spliced\]/$maintxt{'160c'}/g
      ;    # The contents of this Topic have been moved to
    $ssm +=
      $s_s_m =~ s/\[splicedhere\]|\[splithere\]/$maintxt{'160d'}/g; # this Topic
    $ssm += $s_s_m =~
      s/\[split\]/$maintxt{'160e'}/g;    # Off-Topic replies have been moved to
    $ssm += $s_s_m =~ s/\[splithere_end\]/$maintxt{'160f'}/g;    # .
    $ssm +=
      $s_s_m =~ s/\[moved\]/$maintxt{'160'}/g;    # This Topic has been moved to
    $ssm += $s_s_m =~
      s/\[movedhere\]/$maintxt{'161'}/g;    # This Topic was moved here from
    $ssm += $s_s_m =~ s/\[postsmovedhere1\]/$maintxt{'161a'}/g;    # The last
    $ssm += $s_s_m =~
      s/\[postsmovedhere2\]/$maintxt{'161b'}/g;    # Posts were moved here from
    $ssm += $s_s_m =~ s/\[move by\]/$maintxt{'525'}/g;    # by

    if ($ssm) {    # only if it was an internal s_s_m info
        $s_s_m =~
          s~\[link=\s*(\S\w+\://\S+?)\s*\](.+?)\[/link\]~<a href="$1">$2</a>~g;
        $s_s_m =~
          s~\[link=\s*(\S+?)\](.+?)\s*\[/link\]~<a href="http://$1">$2</a>~g;
        $s_s_m =~ s~\[b\](.*?)\[/b\]~<b>$1</b>~g;
        $s_s_m =~ s~\[i\](.*?)\[/i\]~<i>$1</i>~g;
    }
    return ( $s_s_m, $ssm );
...
sub wrap {
    ...
    foreach $cur (@words) {
        ...
        if (   $cur !~ m~\[table(\S*)\](\S*)\[\/table\]~
            && $cur !~ m~\[url(\S*)\](\S*)\[\/url\]~
            && $cur !~ m~\[flash(\S*)\](\S*)\[\/flash\]~
            && $cur !~ m~\[img(\S*)\](\S*)\[\/img\]~ )
        {
            $cur =~ s~(\[\S*?\])~ $1 ~g;
            ...
            foreach $splitcur (@splitword) {
                if (   $splitcur !~ m~www\.(\S+?)\.~
                    ...
                    && $splitcur !~ m~\[\S*\]~ )
                {
                    $splitcur =~ s~(\S{$linewrap})~$1<yabbwrap>~gi;
                }
                $cur .= $splitcur;
            }
        }
...
sub wrap2 {
    $message =~
s#<a href=(\S*?)(\s[^>]*)?>(\S*?)</a># my ($mes,$out,$i) = ($3,"",1); { while ($mes ne "") { if ($mes =~ s/^(<.+?>)//) { $out .= $1; } elsif ($mes =~ s/^(&.+?;|\[ch\d{3,}\]|.)//) { last if $i > $linewrap; $i++; $out .= $1; if ($mes eq "") { $i--; last; } } } } "<a href=$1$2>$out" . ($i > $linewrap ? "..." : "") . "</a>" #eig;
}
...
sub WriteLog {
    ...
    my $user_host =
      ( gethostbyaddr( pack( "C4", split( /\./, $user_ip ) ), 2 ) )[0];
    ...
    if ( !$action && $enableclicklog == 1 ) {
        ...
        print LOG "$field|$date|$ENV{'REQUEST_URI'}|"
          . (
            $ENV{'HTTP_REFERER'} =~ m~$boardurl~i ? '' : $ENV{'HTTP_REFERER'} )
          . "|$ENV{'HTTP_USER_AGENT'}\n";
        foreach (@new_log) {
            if ( ( split( /\|/, $_, 3 ) )[1] >= $onlinetime ) { print LOG $_; }
        }
        fclose(LOG);
    }
}
...
sub RemoveUserOnline {
    ...
    if ($user) {
        ...
        for ( my $i = 0 ; $i < @logentries ; $i++ ) {
            if ( ( split( /\|/, $logentries[$i], 2 ) )[0] ne $user ) {
                print LOG $logentries[$i];
            }
            elsif ( $user eq $username ) {
                $logentries[$i] =~ s/^$user\|/$user_ip\|/;
                print LOG $logentries[$i];
            }
            else { $x = $i; }
        }
...
sub freespace {
    ...
    if ( $FreeBytes >= 1073741824 ) {
        ...
        $yyfreespace =
          sprintf( "%.2f", $FreeBytes / ( 1024 * 1024 ) )
          . " MB ($yyfreespace)";
    }
    else {
        $yyfreespace =
...
sub referer_check {
    return if !$action;
    my $referencedomain =
      substr( $boardurl, 7, ( index( $boardurl, "/", 7 ) ) - 7 );
    my $refererdomain =
      substr( $ENV{HTTP_REFERER}, 7,
        ( index( $ENV{HTTP_REFERER}, "/", 7 ) ) - 7 );
    if (   $refererdomain !~ /$referencedomain/
        && $ENV{QUERY_STRING} ne ""
...
"Can't find required language file. Please inform the administrator about this problem.",
...
sub cloak {
    ...
    for ( $n = 0 ; $n < length $input ; $n++ ) {
        ...
        $ascii = ord($ascii)
          ^ $key;    # xor it instead of adding to prevent wide characters
...
sub guestLangSel {
    ...
    foreach my $filesanddirs ( sort { lc($a) cmp lc($b) } @langDir ) {
        ...
        if (   ( $filesanddirs ne '.' )
            && ( $filesanddirs ne '..' )
            && ( -e "$langdir/$filesanddirs/Register.lng" ) )
        {
            $lngsel = "";
            if ( $filesanddirs eq $language ) {
                $lngsel = qq~ selected="selected"~;
            }
            $langopt .=
              qq~<option value="$filesanddirs"$lngsel>$filesanddirs</option>~;
            $morelang++;
        }
    }
    return $langopt;
...
sub checkUserLockBypass {
    ...
    elsif ( $bypass_lock_perm eq "gmod" && ( $iamadmin || $iamgmod ) ) {
        $canbypass = 1;
    }
    elsif ( $bypass_lock_perm eq "mod" && ( $iamadmin || $iamgmod || $iammod ) )
    {
        $canbypass = 1;
    }
    $canbypass;
        ...
        || ${ $uid . $checkuser }{'position'} eq 'Global Moderator' )
    {
      ...
      usercheck: foreach my $catid (@categoryorder) {
            foreach my $checkboard ( split( /,/, $cat{$catid} ) ) {
                ...
                foreach my $curgroup (
                    split( /, /, ${ $uid . $checkboard }{'modgroups'} ) )
                {
