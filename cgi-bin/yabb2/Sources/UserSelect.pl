...
$userselectplver = 'YaBB 2.6 $Revision: 1.0 $';
...
qq~<div style="float: left; width: 13px; text-align: center; margin-top: 1px; margin-bottom: 1px;"><a href="$scripturl?action=imlist;sort=$INFO{'sort'};toid=$to_id;letter=$page"><span class="small"><b>$showpage</b></span></a></div>~;
    ...
    if ( $INFO{'letter'} && $INFO{'letter'} eq "z" ) {
        $LetterLinks .=
qq~<div style="float: left; width: 11px; text-align: center; border: 1px #ffffff solid;"><span class="small"><b>Z</b></span></div>~;
    }
    else {
        $LetterLinks .=
          ...
          )
        {
            ...
            if ($letter) {
                ...
                if (
                    ...
                    && ( $membername ne $username
                        || ( $to_id =~ /moderators\d/ || $to_id =~ /userspec/ )
                    )
                  )
                {
                    $ToShow[$i] = $membername;
...
qq~<option value="$cloakedUserName"$colorstyle>${$uid.$user}{'realname'}</option>\n~;
        ...
        if ($letter) {
            $yymain .=
              qq~<center><b>$usersel_txt{'noentries'}</b><br /></center>~;
        }
        elsif ( $INFO{'sort'} eq "pmsearch" ) {
            $yymain .=
qq~<center><b>$usersel_txt{'nofound'} <i>$SearchStr</i></b></center>~;
        }
        $yymain .= qq~
            </div>
...
sub buildIndex {
    unless ( $memcount == 0 ) {
        ...
        my ( $pagetxtindex, $pagetextindex, $pagedropindex, $all,
            $allselected );
        $indexdisplaynum = 3;
        $dropdisplaynum  = 10;
        ...
        $pageindex =
qq~<span class="small" style="float: left; height: 21px; margin: 0px; margin-top: 2px;">$usersel_txt{'pages'}: $pagenumb</span>~;
        if ( $pagenumb > 1 || $all ) {
...
qq~<a href="$scripturl?action=imlist;sort=$INFO{'sort'};toid=$to_id;letter=$letter;start=$lastptn" style="font-weight: normal;">$lastpn</a>~;
                ...
                $pagedropindex =
qq~<div style="float: left; width: 456px; height: 21px; margin: 0px; margin-top: 2px; border: 0px;">~;
                $tstart = $start;
...
qq~<option value="$indexstart|$indexend|$MembersPerPage|$indexpage"$selected>$indxoption</option>\n~;
                ...
                $pagedropindex .=
qq~<div id="ViewIndex" class="droppageindex" style="height: 14px; visibility: hidden">&nbsp;</div>~;
...
qq~<br>loadrecentpms from ($pack, $file, $line)<br />=========================~;
    ...
    foreach my $usermessage (@usermessages) {
        ...
        ) = split( /\|/, $usermessage );    # pull name from PM
...
sub checkUserAvail {
    ...
    if ( $INFO{'type'} eq "email" ) {
        ...
        if (
            lc $INFO{'email'} eq
            lc &MemberIndex( "check_exist", $INFO{'email'} ) )
        {
            $taken = "true";
        }
    }
